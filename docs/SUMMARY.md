# Summary

[Introduction](./index.md)

- [Code Style Guide & Skills](./symlink/code-style-guide/skills-and-code-style-contribution.md)
    - [Packages](./symlink/code-style-guide/packages.md)
    - [Basic Syntax](./symlink/code-style-guide/basic-syntax.md)
    - [TypeScript](./symlink/code-style-guide/typescript.md)
    - [Import/Export](./symlink/code-style-guide/import-export.md)
    - [Components](./symlink/code-style-guide/components.md)
    - [Redux](./symlink/code-style-guide/redux.md)
    - [Naming](./symlink/code-style-guide/naming.md)
    - [Tests](./symlink/code-style-guide/tests.md)
    - [Comments](./symlink/code-style-guide/comments.md)
    - [Defensive Programming](./symlink/code-style-guide/defensive-programming.md)
- [Analytics](./analytics/index.md)
    - [AWS](./analytics/aws.md)
    - [Sentry](./analytics/sentry.md)
- [Releases](./releases/index.md)
    - [bundling new firmwares](./releases/bundling-new-firmwares.md)
    - [desktop updates](./releases/desktop_updates.md)
    - [environments](./releases/environments.md)
    - [signing binaries windows](./releases/signing-binaries-win.md)
    - [versioning](./releases/versioning.md)
- [Packages](./packages/index.md)
    - [@trezor/connect](./packages/connect/index.md)
        - [check connect data](./packages/connect/check-connect-data.md)
        - [dependencies](./packages/connect/dependencies.md)
        - [protobuf](./packages/connect/protobuf-type-definitions.md)
        - [coins support](./packages/connect/supported-coins.md)
        - [events](./packages/connect/events.md)
        - [login with Trezor](./packages/connect/login-with-trezor.md)
        - [models](./packages/connect/models.md)
        - [path](./packages/connect/path.md)
        - [methods](./packages/connect/methods/index.md)
    - [@trezor/suite](./packages/suite/index.md)
        - [send form](./packages/suite/send.md)
        - [send form architecture](./packages/suite/send/ARCHITECTURE.md)
    - [@suite/desktop-app](./packages/desktop-app/index.md)
        - [debugging](./packages/desktop-app/debugging.md)
        - [runtime flags](./packages/desktop-app/runtime-flags.md)
        - [anonymous mode](./packages/desktop-app/anon-mode.md)
        - [MCP server](./packages/desktop-app/mcp.md)
            - [Trezor MCP skill](./packages/desktop-app/skills/trezor-mcp/SKILL.md)
                - [Tools reference](./packages/desktop-app/skills/trezor-mcp/references/tools-reference.md)
    - [creating new package](./packages/creating-packages.md)
- [Features](./features/index.md)
    - [coin handler](./features/protocols/coin-handler.md)
    - [transactions export](./features/transactions/export.md)
    - [transactions search](./features/transactions/search.md)
    - [metadata labeling](./features/metadata.md)
    - [fiat rates](./features/fiat-rates.md)
    - [guide](./features/guide.md)
    - [coinjoin](./features/coinjoin.md)
    - [localization](./features/localization.md)
    - [messaging system](./features/message-system.md)
    - [feature flags](./features/feature-flags.md)
    - [application log](./features/application-log.md)
    - [desktop logger](./features/desktop-logger.md)
    - [onboarding](./features/onboarding/onboarding.md)
- [Tests](./tests/index.md)
    - [Suite Playwright E2E](./symlink/tests/e2e-playwright-suite.md)
    - [Suite E2E in CI](./symlink/tests/e2e-ci.md)
    - [Playwright contribution guide](./symlink/tests/e2e-playwright-contribution-guide.md)
    - [GitHub Test Reporter](./symlink/tests/e2e-github-reporter.md)
    - [regtest](./tests/regtest.md)
    - [@suite/test-utils](./tests/suite-test-utils.md)
    - [@suite-common/test-utils](./tests/suite-common-test-utils.md)
    - [@suite-native/test-utils](./tests/suite-native-test-utils.md)
    - [Suite Native E2E performance](./tests/suite-native-e2e-performance.md)
    - [Performance history](./tests/perf-history.md)
- [Miscellaneous](./misc/index.md)
    - [local development security](./misc/local-development-security.md)
    - [build](./misc/build.md)
    - [development on Windows](./misc/development-on-windows.md)
    - [development on Nix](./misc/development-on-nix.md)
    - [device naming](./misc/device-naming.md)
    - [review](./misc/review.md)
    - [videos](./misc/videos.md)
