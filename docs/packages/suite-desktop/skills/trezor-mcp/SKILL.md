...
| HTTP 400 `Invalid JSON-RPC request`               | Body parsed but isn't a JSON object with a `method` string (e.g. `null`, `[]`, `{}`) | Resend a proper JSON-RPC envelope: `{ "jsonrpc": "2.0", "id": ..., "method": "...", "params": {...} }`                                                                                                                                                                                                                                                                                                                   |
| HTTP 400 `Invalid json body: ...`                 | Request body is not valid JSON                                                       | Check that the body is valid JSON and `Content-Type: application/json` is set                                                                                                                                                                                                                                                                                                                                            |
| HTTP 413 `Payload too large`                      | Request body exceeds 1 MB                                                            | Trim the request — MCP requests should be well under 1 MB. If you legitimately need more, split the work across calls                                                                                                                                                                                                                                                                                                    |
| HTTP 404 with empty body                          | No matching route — e.g. `OPTIONS /mcp` (CORS preflight), `PUT /mcp`, or wrong path  | The MCP server only handles `POST/GET/DELETE /mcp`. CORS preflight is intentionally rejected (no CORS headers are sent). Use POST and the exact path `/mcp`                                                                                                                                                                                                                                                              |
| HTTP 500 `Server misconfiguration: no auth token` | MCP enabled in Suite but no token stored                                             | Toggle MCP off and on in Trezor Suite → Settings → Experimental Features to regenerate the token                                                                                                                                                                                                                                                                                                                         |
