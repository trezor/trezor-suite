...
| HTTP 401 Unauthorized                      | Missing or invalid Bearer token                    | The MCP server requires an `Authorization: Bearer <token>` header. The token is auto-generated and shown in Trezor Suite under Settings → Experimental Features → MCP Server. Copy the full config snippet (which includes the token) into your MCP client config. If you recently re-enabled MCP, the token may have changed. If you need to revoke access, use the Regenerate token button in Settings → Experimental Features → MCP Server. |
