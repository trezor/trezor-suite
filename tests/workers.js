...
describe('Worker', () => {
    ...
    it('Worker error (invalid worker factory)', async () => {
        try {
            ...
            expect(error.code).equals('blockchain_link/worker_invalid');
            expect(error.message).equals('Invalid worker object');
        }
    });
    ...
    //         await blockchain.connect();
    //     } catch (error) {
    //         expect(error.code).equals('blockchain_link/worker_timeout');
    //         expect(error.message).equals('Worker timeout');
    //     }
    // }).timeout(35000);
    ...
    it('Worker error (runtime)', async () => {
        try {
            ...
            expect(error.code).equals('blockchain_link/worker_runtime');
        }
    });
});
