...
describe(DropboxProvider.name, () => {
    ...
    it('applies a rate-limit cooldown to requests already waiting in the queue', async () => {
        jest.useFakeTimers();
    ...
    it('uses exponential backoff with jitter when Retry-After is unavailable', async () => {
        jest.useFakeTimers();
