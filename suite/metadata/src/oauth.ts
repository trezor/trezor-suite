...
import { type HttpReceiverAddress, desktopApi } from '@trezor/suite-desktop-api';
 ...
 * For desktop, the HTTP receiver issues a single-use token that must be carried
 * back as the OAuth `state` parameter — providers always echo `state` per RFC 6749.
 * The web fallback returns an empty token (state is generated by the caller).
 */
export const getOauthReceiverUrl = async (): Promise<HttpReceiverAddress | undefined> => {
