...
export const stakingChangeDelegateEvent: EventDef<Attributes, EventType.StakingChangeDelegate> = {
    ...
    descriptionTrigger: 'User navigates through the change delegate/validator flow, with tracking at each step of the process',
    ...
    attributes: {
        action: {
            ...
            description: 'The action taken by the user: `continue` to proceed with changing delegate, `cancel` to abort the process, `close` to dismiss the modal',
        },
        step: {
            changelog: [{ version: '25.4.0', notes: 'added' }],
            description: 'The current step in the change delegate flow: `staking-dashboard` when initiated from the staking dashboard',
        },
        networkSymbol: {
            changelog: [{ version: '25.4.0', notes: 'added' }],
