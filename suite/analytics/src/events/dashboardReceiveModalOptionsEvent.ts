...
> = {
    ...
    attributes: {
        option: {
            changelog: [{ version: '25.10.0', notes: 'added' }],
            description: 'User selection: `account` when selecting a specific account to receive to, `close` when closing the modal, `addAccount` when clicking to add a new account',
        },
        filledSearch: {
            changelog: [{ version: '25.10.0', notes: 'added' }],
            description: 'Whether the user typed in the account search field (`true`) or not (`false`)',
        },
    },
};
