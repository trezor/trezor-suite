import { defineConfig } from '@playwright/test';

import { Model } from '@trezor/trezor-user-env-link';

import { baseConfig } from './playwright-base.config';
import { PlaywrightProjectBuilder } from './playwright-project-builder';
import type { PlaywrightProjectDefinition } from './playwright-project-builder';
import { tagsPr } from './projectTags';
import { PlaywrightTarget } from '../support/testExtends/suiteTestOptions';

/*
 * Web PR All config
 * This config is used when a spec list scopes the PR run (LLM test selector or edited test files).
 * Unlike playwright-web-pr.config.ts, tests are not filtered to @webOnly, T3T1 runs its full set and
 * @optional tests are included — the spec list itself scopes the run. @skipOnPR tests must
 * never run on a PR.
 */
const target = PlaywrightTarget.Web;
const definition: PlaywrightProjectDefinition[] = [
    { model: Model.T3W1, additionalGrepInvert: /@skipOnPR/, currentsTags: tagsPr },
    { model: Model.T3T1, additionalGrepInvert: /@skipOnPR/, currentsTags: tagsPr },
    { model: Model.T3B1, additionalGrepInvert: /@skipOnPR/, currentsTags: tagsPr },
    { model: Model.T2T1, additionalGrepInvert: /@skipOnPR/, currentsTags: tagsPr },
    { model: Model.T1B1, additionalGrepInvert: /@skipOnPR/, currentsTags: tagsPr },
    {
        name: 'no_device',
        additionalGrepInvert: /@skipOnPR/,
        currentsTags: tagsPr,
        grep: /^(?=.*@noDevice)/,
    },
];

const config = defineConfig({
    ...baseConfig,
    projects: PlaywrightProjectBuilder.buildFromDefinitions(target, definition),
});

/* eslint-disable-next-line import/no-default-export */
export default config;
