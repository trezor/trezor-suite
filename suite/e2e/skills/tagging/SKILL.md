---
name: tagging
description: Test tagging and Playwright project selection for Suite E2E tests: device model, platform and execution scope tags. Use when writing or editing tests to control where they run.
---
...
- `@skipOnPR` — never runs on a PR, not even when the LLM test selector or an edited test file targets it. **Required for tests that break or mislead on a PR**: nonce collisions when two runs overlap, old app versions that only the deployed instances provide. Runs in nightly, canary, and release.
- `@optional` — excluded from the full PR run; runs nightly, and on a PR only when the LLM test selector picks it or the PR edits its file. **Use for tests that are low priority and slow.** When the reason is not obvious from the test itself, state it in a one-line comment above the `describe`. Combine with a single platform (`@webOnly` or `@desktopOnly`) to conserve resources.
