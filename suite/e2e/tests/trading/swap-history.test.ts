...
test.describe(
    'Trading - Swap history',
    { tag: ['@webOnly', '@T3T1', '@T3W1', '@optional'] },
    () => {
                ...
                for (const trade of SEEDED_TRADES) {
                    ...
                    await test.step(`Verify detail page for trade ${trade.orderId}`, async () => {
                        ...
                        await expect
                            ...
                            .toHaveText(
                                `${trade.orderId.slice(0, 8)}...${trade.orderId.slice(-8)}`,
                            );

                        await expect(
                            tradingPage.transactionDetailSidebar.sendAccount,
                        ).toContainText(getNetwork(trade.sendSymbol as NetworkSymbol).name);
                        await expect(
                            tradingPage.transactionDetailSidebar.receiveAccount,
                        ).toBeVisible();
                    });
            ...
            async ({ walletPage, tradingPage }) => {
                ...
                await test.step('Open detail for the ongoing (CONFIRMING) trade', async () => {
                    await tradingPage.transactions
                        .transactionRow(PENDING_TRADE.orderId)
                        .root.click();
                    await expect(tradingPage.transactionDetail).toBeVisible();
                });
