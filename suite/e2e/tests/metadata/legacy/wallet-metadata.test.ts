...
test.describe('Metadata - wallet labeling', { tag: ['@webOnly', '@T3W1', '@T3T1'] }, () => {
    ...
    test(
        'persists wallet labels',
        { annotation: createTestAnnotation({ stream: TestStream.Wallet }) },
        async ({ page, device, dashboardPage, metadataPage, devicePrompt, metadataMock }) => {
    ...
    test(
        'labels can be enabled and edited when different wallet is open',
        { annotation: createTestAnnotation({ stream: TestStream.Wallet }) },
        async ({ device, dashboardPage, metadataPage, devicePrompt }) => {
