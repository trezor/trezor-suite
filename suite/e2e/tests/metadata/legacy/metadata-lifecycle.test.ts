...
test.describe('Metadata lifecycle', { tag: ['@webOnly', '@T3W1', '@T3T1', '@optional'] }, () => {
    test.use({
        deviceSetup: {
    ...
    test(
        'Choice persistence and reset behavior',
        ...
        async ({
            ...
            await test.step('re-enable labeling on standard Wallet via settings', async () => {
                ...
                await expect(settingsPage.metadataSelectInput).toHaveTranslation(
                    'TR_LABELING_LEGACY',
                );
            });
        },
