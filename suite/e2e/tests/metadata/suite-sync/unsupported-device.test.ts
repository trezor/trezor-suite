...
test.describe(
    'Suite Sync - Unsupported device banner',
    { tag: ['@T1B1', '@T2T1', '@optional'] },
    () => {
        ...
        test(
            'Banner appears once and does not reappear after dismissal',
            { annotation: createTestAnnotation({ stream: TestStream.Wallet }) },
            async ({ dashboardPage, walletPage, metadataPage }) => {
