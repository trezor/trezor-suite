...
test.describe('Suite Sync - Quota Manager out of quota', { tag: ['@T3W1', '@T3T1'] }, () => {
    ...
    test(
        'Drained device pool shows banner and blocks syncing of a new wallet',
        { annotation: createTestAnnotation({ stream: TestStream.Wallet }) },
        async ({ page, evoluClient, dashboardPage, metadataPage }) => {
...
test.describe('Suite Sync - Quota Manager top-up', { tag: ['@T3W1', '@T3T1'] }, () => {
    ...
    test(
        'Exceeded wallet limit is topped up from the device pool',
        { annotation: createTestAnnotation({ stream: TestStream.Wallet }) },
