...
import { type E2eNetworkSymbol, type ExtractByEventType } from '../../support/types';
...
test.describe('Account types suite', { tag: ['@T3W1', '@T3T1'] }, () => {
    ...
    test(
        ...
        async ({ page, dashboardPage, settingsPage, walletPage }) => {
            const accountTypes: { coin: E2eNetworkSymbol; accounts: { type: string }[] }[] = [
                {
                    coin: 'btc',
                    accounts: [
                        { type: 'normal' },
    ...
    const runNonBtcCoinsTest = async (
        coins: { symbol: E2eNetworkSymbol; path: string }[],
        {
            dashboardPage,
            settingsPage,
            walletPage,
        ...
        const symbolsToEnable = [...new Set(['eth', ...coins.map(c => c.symbol)])];
