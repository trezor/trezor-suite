    ...
    test(
        ...
        async ({
            ...
            await test.step('Wrap ETH to WETH', async () => {
                ...
                await expect(toastSection.wrappedSendAmount).toHaveText(DEPOSIT_AMOUNT);
                await expect(toastSection.wrappedReceiveAmount).toHaveText(DEPOSIT_AMOUNT);
            ...
            await test.step('Approve WETH spending', async () => {
                ...
                await expect(toastSection.approvedAmount).toHaveText('10WETH');
                await expect(yieldFlowSection.pendingTransactionLabel).toHaveTranslation(
            ...
            await test.step('Deposit WETH', async () => {
                ...
                // The confirmed approval advances the wizard to the final deposit step; the
                // collapsed approve step keeps showing the approved allowance as its summary.
            ...
            await test.step('Returning to the dashboard shows the deposited position', async () => {
                ...
                // 8 shares at pricePerShare 1.25 = 10, shown in the native symbol for a
                // wrapped-native vault; yearly reward = 10 × 3.1% APY.
                await expect(yieldSection.depositedAmount(wethPrime.id)).toHaveTranslation(
                    'TR_EARN_YIELD_DASHBOARD_DEPOSITED',
                    { values: { amount: DEPOSIT_AMOUNT, displaySymbol: 'ETH' } },
                );
