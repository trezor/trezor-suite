...
test.describe('Suite Guide', { tag: ['@noDevice'] }, () => {
    ...
    test(
        ...
        async ({ guidePanel }) => {
            ...
            await expect
                .soft(guidePanel.supportFirmwareVersion)
                .toHaveText(FIRMWARE_VERSION_NO_DEVICE);
            await guidePanel.sendBugReport({
                location: 'account',
                report: 'Henlo this is testy test writing hangry test user report',
            });
            await expect(guidePanel.feedbackSuccessToast).toHaveTranslation(
    ...
    test(
        'Open an article and view its screenshot',
        { annotation: createTestAnnotation({ stream: TestStream.Growth }) },
        async ({ guidePanel }) => {
...
test.describe('Suite Guide with device', { tag: ['@T3W1'] }, () => {
    test(
        ...
        { annotation: createTestAnnotation({ stream: TestStream.Growth }) },
        async ({ guidePanel, onboardingPage, page }) => {
            ...
            await test.step('Verify versions with device connected', async () => {
                ...
                await expect
                    .soft(guidePanel.supportFirmwareVersion)
                    .toHaveText(FIRMWARE_VERSION_REGEX);
            });
