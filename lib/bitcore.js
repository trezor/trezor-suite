...
export class BitcoreBlockchain {
    ...
    constructor(endpoints: Array<string>, socketWorkerFactory: SocketWorkerFactory, bufferWorkerFactory: BufferWorkerFactory) {
        ...
        const res = Stream.fromPromise(
            this.socket.promise.then(socket => {
                return lookupAllAddressHistories(
                    ...
                    return Promise.all(r.items.map((item) => historyToMatch(item, this.bufferWorkerFactory)));
                });
            })
        );
    ...
    lookupTransaction(hash: string): Promise<TransactionInfo> {
        return this.socket.promise.then(socket =>
            lookupDetailedTransaction(socket, hash)
                .then((info) => detailedToInfo(info, this.bufferWorkerFactory, false))
        );
    }
    ...
    lookupTransactionAsMatch(hash: string): Promise<TransactionMatch> {
        return this.socket.promise.then(socket =>
            lookupDetailedTransaction(socket, hash)
                .then((info) => detailedToTransactionMatch(info, this.bufferWorkerFactory, true))
        );
    }
...
function historyToMatch(r: BcHistory, factory: BufferWorkerFactory): Promise<TransactionMatch> {
    return detailedToInfo(r.tx, factory, true).then(transaction => {
        return {
            info: transaction,
...
function detailedToInfo(r: BcDetailedTransaction, workerFactory: BufferWorkerFactory, neuter: boolean): Promise<TransactionInfo> {
    return bitcoreToTransaction(r, workerFactory, neuter).then(transaction => {
...
function detailedToTransactionMatch(r: BcDetailedTransaction, worker: BufferWorkerFactory, neuter: boolean): Promise<TransactionMatch> {
    ...
    return detailedToInfo(r, worker, neuter).then(info => {
        return {
            info,
            addresses,
        };
...
function bitcoreToTransaction(r: BcDetailedTransaction, bufferWorkerFactory: BufferWorkerFactory, neutered: boolean = true): Promise<Transaction> {
    return convertBuffer(r.hex, bufferWorkerFactory).then(b => {
        const tx = Transaction.fromBuffer(b);
