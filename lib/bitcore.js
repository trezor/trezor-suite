...
type BcNotification = { address: BcParsedAddress; rejected: boolean; } & BcTransactionInfo;
...
function insightToTransaction(r: InsightTransactionInfo): Transaction {
    ...
    tx.outs = r.vout.map((or): Output => {
        ...
        return {
            address,
            script,
            value: Math.round(parseFloat(or.value) * 1e8),
        };
    });
...
function bitcoreToAddress(r: BcParsedAddress): string {
    // TODO: network
    let script = new Buffer(r.hash, 'hex');
    return baddress.fromOutputScript(script);
}
...
function lookupBlockHash(socket: Socket, height: number): Promise<string> {
    return socket.sendToInsight(`api/block-index/${height}`)
                 .then((r) => r.blockHash);
}

// requires insight-api bitcore-node plugin
function lookupSyncStatus(socket: Socket): Promise<BcSyncStatus> {
    return socket.sendToInsight(`api/sync`);
}
...
function lookupInsightTransaction(
    ...
    return socket.sendToInsight(`api/tx/${hash}`);
}
