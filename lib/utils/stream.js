...
export class Emitter<T> {
    ...
    destroy() {
        this.listeners.forEach(listener => this.detach(listener));
        this.listeners = [];
    }
...
type Ender<T> = (t: T) => void;
type EndingController<UpdateT, EndingT> = (update: Updater<UpdateT>, finish: Ender<EndingT>, finishError: Ender<Error>) => Disposer;
...
export class StreamWithEnding<UpdateT, EndingT> {
    ...
    static fromController(e: EndingController<UpdateT, EndingT>): StreamWithEnding<UpdateT, EndingT> {
        const res = new StreamWithEnding();

        const def: Deferred<EndingT> = deferred();
        const emitter: Emitter<UpdateT> = new Emitter();
        const hisDisposer = e(
        ...
        const disposer = (e: Error) => {
            hisDisposer(e);
            def.reject(e);
        };

        const stream: Stream<UpdateT> = new Stream((update, finish) => {
            let finished = false;
            const handler = (t) => {
                if (!finished) {
                    update(t);
            ...
            emitter.attach(handler);
            def.promise.then(promiseHandler);
            def.promise.catch(promiseHandler);

            return () => {
                finished = true;
                def.reject(new Error('Stream disposed.'));
                emitter.detach(handler);
            };
        });

        res.stream = stream;
        res.ending = def.promise;
        res.dispose = disposer;
        return res;
    }
}
