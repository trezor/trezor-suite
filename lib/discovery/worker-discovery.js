...
export class WorkerDiscovery {
    ...
    createAddressSource(node: BitcoinJsHDNode, network: BitcoinJsNetwork, segwit: 'off' | 'p2sh'): CachingAddressSource {
        let source;
        const version = segwit === 'off' ? network.scriptHash : network.pubKeyHash;
        source = new WorkerAddressSource(this.addressWorkerChannel, node, network.pubKeyHash, segwit);
