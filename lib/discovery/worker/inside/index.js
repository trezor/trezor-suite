...
channel.startDiscoveryPromise.then(() => {
    ...
    loadBlockRange(initialState).then(range => {
        ...
        return discoverAccount(range, [lastUsedMain, lastUsedChange], oldState.transactions)
...
function discoverAccount(
    ...
    return Promise.all([
        new GetChainTransactions(0, range, lastUsedAddresses[0], channel.chunkTransactions, transactions).discover(),
        new GetChainTransactions(1, range, lastUsedAddresses[1], channel.chunkTransactions, []).discover(),
