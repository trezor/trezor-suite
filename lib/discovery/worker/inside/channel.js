...
export const initPromise: Promise<{accountInfo: ?AccountInfo, network: BitcoinJsNetwork, xpub: string, segwit: boolean}> = initDfd.promise;

messageEmitter.attach((message, detach) => {
    if (message.type === 'init') {
        detach();
        initDfd.resolve({accountInfo: message.state, network: message.network, xpub: message.xpub, segwit: message.segwit});
    }
});
