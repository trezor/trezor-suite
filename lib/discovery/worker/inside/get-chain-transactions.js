...
export class GetChainTransactions {
    ...
    originalTransactions: {[address: string]: number};

    // a little hack
    // when I am chain 0, I communicate a number, that is vaguely correlated with
    // "loaded transactions", to the main thread, so he can display something
    // It is not really important 100% what do I show up and when, just that it
    // more or less correlates with the transaction count and that it increases
    // as the discovery goes on
    ...
    constructor(
        ...
        this.originalTransactions = deriveCounts(originalTransactions);
    ...
    handleTransactions(
        ...
        this.allCheckedAddresses.forEach(address => {
            this.pseudoCount += this.originalTransactions[address] || 0;
        });
        ...
        this.pseudoCount += Object.keys(this.newTransactions).length;
    }
