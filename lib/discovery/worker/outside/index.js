...
export class WorkerDiscoveryHandler {
    ...
    discovery(ai: ?AccountInfo): StreamWithEnding<AccountLoadStatus, AccountInfo> {
        this.workerChannel.postToWorker({type: 'init', state: ai, network: this.network});
    ...
    getChunkStream(
        ...
        const addressPromise = addresses == null ? source.derive(firstIndex, lastIndex) : Promise.resolve(addresses);
