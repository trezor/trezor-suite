...
export class Stream<T> {
    ...
    static repeatWithTimeout(
        promise: () => Promise<T>,
        ms: number
    ): Stream<T> {
        const nulls: Stream<?T> = Stream.generate(
            null,
            () => promise().then(res => new Promise((resolve) => setTimeout(() => resolve(res), ms))),
            () => true
        );
        return Stream.filterNulls(nulls);
    }
    ...
    // note: this doesn't keep the order
