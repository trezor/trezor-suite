...
var getLatestSafeFw = function getLatestSafeFw(input) {
  ...
  if (isInBootloader) {
    var filteredList = getSafeBlVersions(releasesList, bootloaderVersion);
  ...
  if (!isInBootloader) {
    var _filteredList = getSafeFwVersions(releasesList, firmwareVersion);
    if (_filteredList.length > 0) {
      return _filteredList[_filteredList.length - 1];
    }
