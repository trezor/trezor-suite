...
export class PromiseIterable<T> {
    ...
    static fromIterable(source: Iterable<T>): PromiseIterable<T> {
        return new PromiseIterable(() => {
            ...
            const state: Iterator<T> = source[Symbol.iterator]();
            return () => {
                const next = state.next();
                const nvalue = next.value;
                /* $FlowIssue
                 ...
                 */
                const value: ?Value<T> = next.done ? null : new Value(nvalue);
    ...
    last(): Promise<T> {
        return this.resolveAll().then(list => {
            if (list.size === 0) {
    ...
    map<U>(fn: (t: T, i: number) => U): PromiseIterable<U> {
        let i = -1;
        return new PromiseIterable(() => {
            const state: PromiseIterator<T> = this.iterator();
            return () => {
    ...
    static flatten(arrays: PromiseIterable<Array<T>>): PromiseIterable<T> {
        return new PromiseIterable(() => {
            const state: PromiseIterator<Array<T>> = arrays.iterator();
            const lastArr: Array<T> = [];
            return () => {
                if (lastArr.length !== 0) {
                    return Promise.resolve(new Value(lastArr.shift()));
                }
                const fun = () => state.next().then((array) => {
                    if (array != null) {
    ...
    chunkWhile(addMore: ((ts: Array<T>) => boolean)): PromiseIterable<Array<T>> {
        return new PromiseIterable(() => {
            const state: PromiseIterator<T> = this.iterator();
            ...
            return () => {
                ...
                const fun = () => state.next().then((value) => {
                    if (value != null) {
                        arr.push(value.value);
                        if (addMore(arr)) {
                            return fun();
                        } else {
                            const res = new Value(arr);
                            arr = [];
                            return res;
                        }
                    } else {
    ...
    chunk(size: number): PromiseIterable<Array<T>> {
        return this.chunkWhile(arr => arr.length < size);
    }
