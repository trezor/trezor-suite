...
type Collection<T> = Array<T> | ImmutableSet<T> | ImmutableMap<any, T>;

export type TransactionMapData = Array<TransactionInfoData>;

export class TransactionMap {
    infos: ImmutableMap<string, TransactionInfo>;

    constructor(infos: ImmutableMap<string, TransactionInfo> = new ImmutableMap()) {
        this.infos = infos;
    }
    ...
    set(info: TransactionInfo) {
        this.infos = this.infos.set(info.id, info);
    }
