...
class ChainDiscovery extends EventEmitter {
    ...
    start() {
        ...
        // use timestamps to avoid the overhead. also, we need to catch the
        // reorg case and load the whole history
...
class AccountDiscovery extends EventEmitter {
    ...
    constructor(sources, blockchain, chunkSize, gapLength) {
        ...
        this.historyTreshold = this.discoveries.length;
    }
    ...
    update() {
        ...
        // TODO: we need to check the received histories are from the correct
        // chains. like this, a hangup on 2nd branch and 2 fast histories on the
        // 1st branch emit a wrong result.

        this.receivedHistories += 1;

        if (this.receivedHistories >= this.historyTreshold) {
