...
import bitcoinJsSplit from 'coinselect/src/outputs/split';
import bitcoinJsCoinselect from 'coinselect';
import {transactionBytes} from 'coinselect/src/utils';

import type {UtxoInfo} from '../discovery';
import * as request from './request';

const SEGWIT_INPUT_LENGTH = 91;
const INPUT_LENGTH = 149;
const OUTPUT_LENGTH = 34;
...
export function coinselect(
    ...
    rOutputs: Array<request.Request>,
    height: number,
    feeRate: number,
    segwit: boolean,
    countMax: boolean,
    ...
    if (!result.inputs) {
        ...
        const totalSpent = result.outputs.filter((output, i) => {
            // change is always without script
            return output.script != null;
        }).map(o => o.value).reduce((a, b) => a + b, 0);
...
function convertInputs(
    ...
    const bytesPerInput = segwit ? SEGWIT_INPUT_LENGTH : INPUT_LENGTH;
...
function convertOutputs(
    outputs: Array<request.Request>
): Array<OutputIn> {
    const script = {length: OUTPUT_LENGTH};
