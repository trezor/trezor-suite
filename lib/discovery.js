...
export type AccountDiscoveryDataWithTransactions = {
    ...
    transactions: PromiseIterable<{iterable: PromiseIterable<TransactionInfoData>, size: number}>;
};
...
export function storeAccountDiscovery(
    ...
    const transactions = PromiseIterable.fromIterable(account).map(account => ({
        iterable: PromiseIterable.fromIterable(account.transactions.values()).map(tx => tx.toJSON()),
        size: account.transactions.size,
    }));
