...
export function restoreAccountDiscovery(
    data: AccountDiscoveryDataWithTransactions,
    sources: Array<CachingSource>
): Promise<AccountDiscoveryState> {
    data.data.sources.forEach((sdata, i) => {
        sources[i].restore(sdata);
    });
    let chain = newChain();
    let blocks = data.data.blocks;
    let transactions = new TransactionMap();
    return promiseAllSequentially(data.data.transactionIds.map(id => () =>
        data.getTransaction(id).then(transaction => transactions.set(id, TransactionInfo.fromJSON(transaction)))
    )).then(() => {
