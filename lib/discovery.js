...
export function storeAccountDiscovery(
    ...
    let transactions = PromiseIterable.fromIterable(txs.values()).map(tx => tx.toJSON());
...
export function restoreAccountDiscovery(
    ...
    return transactions
        ...
        .map(tx => transactionsMap.set(tx.id, tx))
        .resolveAll()
        .then(() => {
            return data.histories.map((history) => {
                return {
                    ...
                    blocks,
                    chain,
                    transactions: new ImmutableMap(transactionsMap),
                };
            });
        });
}
