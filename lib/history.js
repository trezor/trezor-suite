...
export function analyzeTransaction(
    ...
    let isExternal = (a) => a && chainContainsAddress(external, a);
    let isInternal = (a) => a && chainContainsAddress(internal, a);
    let isCredit = (a) => isExternal(a) || isInternal(a);
    let isDebit = (a) => !isCredit(a);
    ...
    tx.ins.forEach((i, index) => {
        let info = transactions.get(inputIds[index]);
        if (info) {
            let o = info.tx.outs[i.index];
            let a = info.outputAddresses[i.index];
            if (o && isCredit(a)) {
                value -= o.value;
                nDebit++;
            }
    ...
    if (nDebit === tx.ins.length && nCredit === tx.outs.length) {
        ...
        type = 'incoming';
        targets = tx.outs.filter((o, i) => isExternal(outputAddresses[i]));
        if (targets.length === 0) {
            targets = tx.outs.filter((o, i) => isInternal(outputAddresses[i]));
        }

    } else {
        // outgoing transaction, targets are debit outputs
        type = 'outgoing';
        targets = tx.outs.filter((o, i) => isDebit(outputAddresses[i]));
    }
