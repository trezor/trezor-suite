...
describe('TradingHistoryDetailInfo', () => {
    ...
    it('renders buy amounts, payment method, provider, and destination account', async () => {
        const { getByLabelText, getByTestId, getByText } = await renderInfo(
        ...
        expect(
            getByTestId('@trading/history/detail/info/pay/asset/primary-label'),
        ).toHaveTextContent('USD');
        expect(getByTestId('@trading/history/detail/info/pay/asset/amount')).toHaveTextContent(
            '1,234',
        );
        expect(
            getByTestId('@trading/history/detail/info/get/asset/primary-label'),
        ).toHaveTextContent('ETH');
        expect(getByTestId('@trading/history/detail/info/get/account-label')).toHaveTextContent(
            'to ETH Account #1',
        );
    });
