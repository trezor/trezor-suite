import { SkPath, Skia, Vector, cartesian2Polar } from '@shopify/react-native-skia';
...
export const pixelFactorX = (date: Date, minValue: Date, maxValue: Date): number => {
...
export const pixelFactorY = (value: number, minValue: number, maxValue: number): number => {
...
function createGraphPathBase({
    ...
    const getGraphPoint = (point: GraphPoint): Vector => {
        const x =
            actualWidth * pixelFactorX(point.date, range.x.min, range.x.max) + horizontalPadding;
    ...
    points.forEach((point, i) => {
        ...
        const splineFunction = createSplineFunction(prevPoint, cps, cpe, currentPoint);
