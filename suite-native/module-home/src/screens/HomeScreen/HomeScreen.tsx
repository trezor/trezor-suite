...
import {
    selectIsDeviceDiscoveryEmptyAndAccountless,
...
export const HomeScreen = () => {
    const isDeviceDiscoveryEmptyAndAccountless = useSelector(
        selectIsDeviceDiscoveryEmptyAndAccountless,
    );
    ...
    const isEmptyHomeRendererShown =
        isDeviceDiscoveryEmptyAndAccountless && // There has to be no accounts and discovery not active.
    ...
    const refreshControl = useHomeRefreshControl({
        isPortfolioEmpty: isDeviceDiscoveryEmptyAndAccountless,
        portfolioContentRef,
    });
