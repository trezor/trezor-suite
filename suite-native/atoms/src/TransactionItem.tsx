...
const transactionSignStyle = prepareNativeStyle<{ transactionStatus: TransactionStatus }>(
    (utils, { transactionStatus }) => ({
        ...
        extend: [
            {
                condition: transactionStatus === 'received',
                style: { color: utils.colors.textPrimaryDefault },
            },
            {
                condition: transactionStatus === 'sent',
                style: { color: utils.colors.textAlertRed },
            },
        ],
    }),
);
    ...
    return (
        <Box style={applyStyle(transactionItemWrapperStyle)}>
            <Box style={applyStyle(iconWrapperStyle)}>
                <Icon name="receive" color="iconSubdued" size="medium" />
            </Box>
