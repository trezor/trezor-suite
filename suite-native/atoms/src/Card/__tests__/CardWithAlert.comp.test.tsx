...
import { CardWithAlert, CardWithAlertProps } from '../CardWithAlert';

describe('CardWithAlert', () => {
    const renderComponent = (props: Omit<CardWithAlertProps, 'children'>) => {
        const cardWithAlertProps = {
            children: <Text>hello</Text>,
            ...props,
        } as CardWithAlertProps;

        return renderWithBasicProvider(<CardWithAlert {...cardWithAlertProps} />);
    };
    ...
    it('should not reset border radiuses if alert is not present', () => {
        ...
        expect(queryByTestId('@atom/card/content')).not.toHaveStyle({ borderTopLeftRadius: 0 });
        expect(queryByTestId('@atom/card/content')).not.toHaveStyle({
