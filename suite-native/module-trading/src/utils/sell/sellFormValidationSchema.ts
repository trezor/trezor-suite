...
import { SellFormContext } from '../../types/sell';

const getAmountLimitContext = ({
    options,
}: yup.TestContext<unknown>): Omit<SellFormContext, 'currency'> & {
    currency: string;
} => {
    const context = options.context as SellFormContext;
...
const formatCryptoAmount = (
    amount: string,
    currency: string,
    CryptoAmountFormatter: SellFormContext['CryptoAmountFormatter'],
) =>
