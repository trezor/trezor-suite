...
describe('tradingSlice', () => {
    ...
    describe('sendFormDraft', () => {
        ...
        it('setTradingSendFormDraft should set sendFormDraft', () => {
            const state = tradingReducer(
                undefined,
                tradingSlice.actions.setTradingSendFormDraft(mockFormState),
            );

            expect(state.sendFormDraft).toEqual(mockFormState);
        });

        it('setTradingSendFormDraft should update existing sendFormDraft', () => {
            ...
            const actions = [
                tradingSlice.actions.setTradingSendFormDraft(mockFormState),
                tradingSlice.actions.setTradingSendFormDraft(updatedFormState),
            ];

            const state = actions.reduce(tradingReducer, undefined) as TradingState;

            expect(state.sendFormDraft).toEqual(updatedFormState);
        });

        it('clearTradingSendFormDraft should set sendFormDraft to undefined', () => {
            const actions = [
                tradingSlice.actions.setTradingSendFormDraft(mockFormState),
                tradingSlice.actions.clearTradingSendFormDraft(),
            ];

            const state = actions.reduce(tradingReducer, undefined) as TradingState;

            expect(state.sendFormDraft).toBe(undefined);
        });
    });
});
