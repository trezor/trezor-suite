...
export type TokenLogoSize = keyof typeof tokenLogoSizes;
...
const networkWrapperStyle = prepareNativeStyle<{ size: TokenLogoSize }>((utils, { size }) => ({
    position: 'absolute',
    right: 0,
    bottom: 0,
...
const TokenLogoComponent = ({ symbol, contractAddress, size = 'small' }: TokenLogoProps) => {
    const { applyStyle } = useNativeStyles();
    const { translate } = useTranslate();

    const sizeNumber = tokenLogoSizes[size];
...
export const TokenLogo = ({
    ...
    if (!showNetworkIcon) {
        return <TokenLogoComponent symbol={symbol} contractAddress={contractAddress} size={size} />;
    }
    ...
    const iconSymbol = contractAddress ? symbol : getNetwork(symbol).symbol;

    return (
        <View style={{ width: tokenLogoSizes[size], height: tokenLogoSizes[size] }}>
            <TokenLogoComponent symbol={iconSymbol} contractAddress={contractAddress} size={size} />
            {shouldShowNetwork && (
