...
export const useHandleDeviceConnection = () => {
    ...
    const isDeviceStackFocused = lastRoute === RootStackRoutes.DeviceStack;
    ...
    useEffect(() => {
        if (isOnboardingFinished && hasDeviceRequestedPin && !isDeviceStackFocused) {
            navigation.navigate(RootStackRoutes.AuthorizeDeviceStack, {
                screen: AuthorizeDeviceStackRoutes.PinMatrix,
            });
        }
    }, [hasDeviceRequestedPin, isDeviceStackFocused, isOnboardingFinished, navigation]);
};
