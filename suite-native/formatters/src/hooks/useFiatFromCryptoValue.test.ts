...
const createRate = (rate: number): Rate => ({
    ...
    isLoading: false,
    error: null,
    ticker: { symbol: asNetworkSymbol('btc') },
});
...
describe(useFiatFromCryptoValue.name, () => {
    ...
    it('converts a crypto balance to a display-rounded fiat amount', async () => {
        setNewStoreMockup({
            currentRates: { [getFiatRateKey(asNetworkSymbol('btc'), 'usd')]: createRate(12.528) },
        });
    ...
    it('returns the same instance while the rendered amount is unchanged', async () => {
        setNewStoreMockup({
            currentRates: { [getFiatRateKey(asNetworkSymbol('btc'), 'usd')]: createRate(12.523) },
        });
        ...
        setNewStoreMockup({
            currentRates: { [getFiatRateKey(asNetworkSymbol('btc'), 'usd')]: createRate(12.5238) },
        });
    ...
    it('converts a token balance using the token rate', async () => {
        ...
        setNewStoreMockup({
            currentRates: {
                [getFiatRateKey(asNetworkSymbol('eth'), 'usd', tokenAddress)]: createRate(2.503),
            },
        });
        const { result } = await renderUseFiatFromCryptoValue({
            cryptoValue: '1000000',
    ...
    it('returns null for a testnet symbol', async () => {
        setNewStoreMockup({
            currentRates: { [getFiatRateKey(asNetworkSymbol('test'), 'usd')]: createRate(12.523) },
        });
        const { result } = await renderUseFiatFromCryptoValue({
            cryptoValue: '1',
