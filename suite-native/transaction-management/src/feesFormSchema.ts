...
const validateFeeDecimalLength = (
    networkConfigDeps: NetworkConfigDeps,
    value: string | undefined,
    context: Partial<FeesFormContext>,
) => {
    ...
    const { networkType } = getNetwork(networkConfigDeps, symbol);

    if (!networkFeeInfo || !networkType) return false;

    const maxDecimals = getFeeDecimals(networkConfigDeps, { symbol });
...
export const feesFormValidationSchema = (networkConfigDeps: NetworkConfigDeps) =>
    yup.object({
        ...
        customFeePerUnit: yup
            ...
            .test('too-many-decimals', 'Too many decimals.', function (value) {
                const { translate, isEip1559Fee, ...context } = this.options
                    .context as FeesFormContext;
                if (!value || isEip1559Fee) return true;
                if (!validateFeeDecimalLength(networkConfigDeps, value, context)) {
        ...
        customFeeLimit: yup.string().test(
            ...
            function (value) {
                if (!value) return true;
                const { symbol, minimalFeeLimit, translate } = this.options
                    .context as FeesFormContext;
        ...
        customMaxFeePerGas: yup
            ...
            .test('max-fee-per-gas-range', 'Max fee per gas is out of range.', function (value) {
                ...
                const { translate, networkFeeInfo, symbol } = this.options
                    .context as FeesFormContext;
...
export type FeesFormValues = yup.InferType<ReturnType<typeof feesFormValidationSchema>>;
