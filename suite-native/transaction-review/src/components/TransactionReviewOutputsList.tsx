        ...
        <>
            <VStack spacing="sp16">
                {review.outputs.map((output, index) => {
                    ...
                    if (review.renderOutputItem) {
                        return (
                            <Fragment key={getOutputKey(output)}>
                                {review.renderOutputItem({ output, onLayout })}
                            </Fragment>
                        );
                    }
            ...
            {!review.isTransactionAlreadySigned &&
                (review.isSlidingOverlayEnabled ?? !isSolana) && (
