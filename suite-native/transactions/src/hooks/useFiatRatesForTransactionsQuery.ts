...
import { selectBaseCurrency, updateTxsFiatRatesThunk } from '@suite-common/wallet-core';
import {
    type AccountKey,
...
import { readHistoricRates, writeHistoricRates } from '../historicRatesStorage';
...
export const useFiatRatesForTransactionsQuery = ({
    ...
    // Seed TQ cache from MMKV synchronously before the first queryFn fires.
    // useLayoutEffect runs before TQ's useEffect-based query scheduling, so the
    // cache is guaranteed to have persisted rates when txFiatRates.queryFn checks it.
    useLayoutEffect(() => {
        if (!queryClient.getQueryData(mobileQueryKeys.historicRates(localCurrency))) {
            const persisted = readHistoricRates(localCurrency);
            if (persisted) {
                queryClient.setQueryData(mobileQueryKeys.historicRates(localCurrency), persisted);
            }
        }
    }, [localCurrency, queryClient]);
        ...
        queryFn: async () => {
            const existingRates =
                queryClient.getQueryData<RatesByTimestamps>(
                    mobileQueryKeys.historicRates(localCurrency),
            ...
            const txsNeedingRates = transactions.filter(tx => {
                ...
                return tx.tokens.some(
                    token =>
                        !existingRates[
                            getFiatRateKey(tx.symbol, localCurrency, token.contract as TokenAddress)
                        ]?.[ts],
                );
            });
            ...
            startTransition(() => {
                queryClient.setQueryData<RatesByTimestamps>(
                    ...
                    (prev = {} as RatesByTimestamps) => {
                        const merged = { ...prev };
                        for (const [key, timestamps] of typedObjectEntries(newRates)) {
            ...
            if (persisted) {
                writeHistoricRates(localCurrency, persisted);
            }
