...
describe('tradeableAssetUtils', () => {
    describe('coinInfoToTradeableAsset', () => {
        ...
        it('should transform native ETH network correctly', () => {
            ...
            expect(
                coinInfoToTradeableAsset(networkConfigDeps, cryptoId as CryptoId, coins[cryptoId]),
            ).toEqual({
    ...
    describe('getSymbolFromTradeableAsset', () => {
        it.each([
            ...
            expect(getSymbolFromTradeableAsset(networkConfigDeps, asset)).toEqual(expectedSymbol);
        });
    });
});
