...
describe('coinselect bnb', () => {
    fixtures.forEach((f) => {
        it(f.description, () => {
            ...
            const actual = coinAccum(f.factor)(inputs, outputs, f.feeRate, {inputLength, outputLength});

            assert.deepEqual(actual, expected);
            if (actual.inputs) {
                const feedback = coinAccum(f.factor)(actual.inputs, actual.outputs, f.feeRate, {inputLength, outputLength});
