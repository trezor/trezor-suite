...
const run = async () => {
        ...
        knownCredentials = [
            // Trezor
            {
                trezor_static_public_key:
                    '96d6707933b418109eb125acedda79c178419ea47b1028644a6d89a8491a0d4d',
                credential:
                    '0a1b0a08746573742065326510001a0d5472657a6f72436f6e6e6563741220913ce265725711a531efbb2ff67a238895ae0f205d6eaa5701adef29c5a822a5',
    ...
    if (argv.bluetooth) {
        ...
        const btDevice = enumerate.devices.find(d => d.paired);
        if (!btDevice) {
            throw new Error('BT Device is missing');
        }

        await bluetoothApi.send('connect_device', { id: btDevice.id, timeout: 15000 });
