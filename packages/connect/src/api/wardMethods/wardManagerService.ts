...
/**
 * WardManagerService — the freshness-authority seam (the WARD Manager / WM).
 *
 * OWNS: producing the WM's Ed25519 signatures the device verifies — over `wardId`
 * for both the sync-round attestation and the commit-round candidate.
 * MUST NOT: touch the host database or the device; it is a pure signer.
 * SEAMS: `MockWardManagerService` (dev/test, debug key, in-process) and
 * `HttpWardManagerService` (real, out-of-process). Swap via `setWardManagerService`.
 *
 * TODO(handoff, gap 2): today `signAttestation` signs the host-supplied
 * `(counter, mac)` — a signing oracle. A hardened WM must load and sign its OWN
 * authoritative stored head for `wardId` (input becomes `{ wardId, nonce }`,
 * output should carry the head it signed). See gaps.md #2 and the server
 * `createAttestHandler.ts`.
 */
