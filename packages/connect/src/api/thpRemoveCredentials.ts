...
export default class ThpRemoveCredentials extends AbstractMethod<'thpRemoveCredentials'> {
    ...
    run() {
        console.warn('->> ThpRemoveCredentials run');
        ...
        // should @trezor/connect remember that this device is requested not to be remembered? this will work until device disconnection or @trezor/connect lifecycle

        thpState.resetState(); // reset device state

        // followup: increase credentials counter on device
