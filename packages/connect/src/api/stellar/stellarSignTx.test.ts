...
// `transfer` with one signed auth entry, soroban data and a nonce wider than MAX_SAFE_INTEGER.
const SOROBAN_XDR =
...
describe(stellarSignTx.name, () => {
    it('sends the contract invocation and the transaction extension the device asks for', async () => {
        ...
        const transaction: StellarTransaction = {
            ...
            sorobanData: '0000000100000002deadbeef',
        };
    ...
    it('refuses to sign when the soroban data the device asked for is missing', async () => {
        ...
        ).rejects.toThrow('missing sorobanData');
    });
    ...
    it('signs a transaction parsed straight from a dApp envelope', async () => {
        ...
        const envelope = parseTransactionFromXDR(SOROBAN_XDR, false).toEnvelope();
        ...
        if (ext.type !== 'sorobanData') {
            throw new Error('expected the transaction to carry soroban data');
        }

        expect(sorobanData).toBe(ext.sorobanData.toXdr('hex'));
    });
