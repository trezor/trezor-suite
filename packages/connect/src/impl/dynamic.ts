...
export class TrezorConnectDynamicImpl implements ConnectFactoryDependencies {
    ...
    private impls: ConstructorParas['impls'];
    private getEnv: ConstructorParas['getEnv'];

    private lastSettings?: Partial<ConnectSettings>;

    public constructor({ impls, getEnv }: ConstructorParas) {
        this.impls = impls;
        this.getEnv = getEnv;
        this.eventEmitter = new ProxyEventEmitter(this.impls.map(impl => impl.impl.eventEmitter));
    }

    private getTarget() {
        return this.impls.find(impl => impl.type === this.currentTarget)!.impl;
    }
