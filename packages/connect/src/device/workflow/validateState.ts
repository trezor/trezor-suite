...
const getInvalidThpDeviceState = async (context: WorkflowContext) => {
    ...
    if (expectedSessionId) {
        ...
        thpState?.setSessionId(Uint8Array.from([Number(expectedSessionId)]));
        uniqueState = await getStaticSessionId(device).catch(e => {
            if (e.code === 'Failure_InvalidSession') {
                // TODO: catch specific thp errors
                // requested sessionId is not valid, reset state
                device.setState({
                    sessionId: undefined,
                    deriveCardano: undefined,
                });
                // device.thp.state.resetState();
                thpState?.setSessionId(new Uint8Array(1));
    ...
    if (!uniqueState || (!currentState?.deriveCardano && method.useCardanoDerivation)) {
        const knownSessions = device.getAllInstances().map(s => s.sessionId);
        let newSessionId = 0;
        ...
        thpState?.setSessionId(sessionId);
