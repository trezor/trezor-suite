...
export async function thpCall(
    ...
    console.warn('THPCall', name, device.protocolState);
...
const thpWaitForThpPairingTag = async (device: Device) => {
    ...
    console.warn('thpWaitForThpPairingTag', device.protocolState.handshakeCredentials);
    ...
    promptThpPairing(device)
        ...
        .catch(e => {
            console.warn('catch promptThpPairing error', e);
            dfd.resolve({ error: e.message });
        });

    const pairingResponse = await dfd.promise;
    readAbort.abort();
    console.warn('Waiting for read cancel to finish-start');
    await readCancel;
    console.warn('Waiting for read cancel to finish-end');
    ...
    if (!device.features.passphrase_protection) {
        ...
        passphrase = await promptPassphrase(device)
            .then(response => {
                ...
                console.warn('getThpSession', response);

                return response;
            })
            .catch(e => {
                console.warn('getThpSession error', e);
            });
    }
...
const thpPairing = async (
    ...
    console.warn('my new credentials!', pairedCredentials.message);
...
export const initThpChannel = async (device: Device, settings?: ThpSettings) => {
    ...
    if (!device.features) {
        console.warn('Start handshake...', device.protocolState);
        const handshake = await thpHandshake(device, settings);
        console.warn('Handshake success', handshake);
