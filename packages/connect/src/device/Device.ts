...
import * as Messages from '@trezor/transport/lib/types/messages';
...
class Device extends EventEmitter {
    ...
    // REF-TODO: A required parameter cannot follow an optional parameter
    // @ts-ignore
    async _runInner<X>(fn?: () => Promise<X>, options: RunOptions): Promise<void> {
    ...
    async initialize(useEmptyPassphrase: boolean, useCardanoDerivation: boolean) {
        let payload: Messages.Initialize;
        if (this.features) {
        ...
        // REF-TODO - no refactoring during transfer
        // @ts-ignore
    ...
    _updateFeatures(feat: Features) {
        ...
        if (versionCompare(version, this.getVersion()) !== 0 || capabilitiesDidChange) {
            this.unavailableCapabilities = getUnavailableCapabilities(
                feat,
                getAllNetworks(),
                // REF-TODO: infer type from config and use it in deviceFeaturesUtils ?
                // @ts-ignore
