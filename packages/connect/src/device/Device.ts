        ...
        if (this.protocol.name === 'v2') {
            if (expectedSessionId) {
                ...
                deviceState = await this.getCommands()
                    ...
                    .catch(e => {
                        if (e) {
                            console.warn('Verify ThpSession error', e);
            ...
            if (!deviceState) {
                this.protocolState.setSessionId(0);
                console.warn('-------------> Well... i need new ThpSession');
                const sessionId = await getThpSession(this);
                this.setState({ sessionId: sessionId.toString() }); // TODO string/number?
