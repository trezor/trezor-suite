...
// TODO: do we have some enum utility for that?
const enumFromString = (dm: ThpPairingMethod | keyof typeof ThpPairingMethod) => typeof dm === 'string' ? protocolThp.ThpPairingMethod[dm] : dm;
const getPairingMethods = (
    ...
    deviceMethods?.flatMap(dm => {
        ...
        const isRequested =
            settingsMethods &&
            settingsMethods.find(sm => value === enumFromString(sm));
...
export const createThpChannel = async (device: Device, settings?: ThpSettings) => {
    ...
    if (!pairingMethods?.length) {
        throw new Error('No common pairing methods'); // TODO: some typed error?
    }
...
export const thpHandshake = async (device: Device, settings?: ThpSettings) => {
    ...
    // do a lot of cryptography steps from .. to ..
