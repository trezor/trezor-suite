    ...
    readCancel
        .then(r => {
            if (r.success) {
                ...
                // @ts-expect-errorThpSelectMethod
                dfd.resolve({ error: r.payload.message.message });
                // again this is confusing error/code/message
            }
        })
        .catch(() => {
            // silent
...
export const thpPairing = async (device: Device, settings: ThpSettings) => {
    const thpState = device.getThpState();
    if (!thpState?.handshakeCredentials) {
        throw ERRORS.TypedError('Device_ThpStateMissing');
    }

    console.warn('thpPairing1', thpState.isPaired, thpState.pairingMethod);
    ...
    console.warn('thpPairing2', thpState.isPaired);
