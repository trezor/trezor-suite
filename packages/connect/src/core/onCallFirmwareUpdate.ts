...
const getInstallationParams = (device: Device, params: Params) => {
    ...
    if (!device.features.bootloader_mode) {
        ...
        const isUpdatingToNewerVersion = !version
            ? device.firmwareReleaseConfig?.isNewer
            : isNewer(version, [
                  device.features.major_version,
                  device.features.minor_version,
                  device.features.patch_version,
...
const getBinaryHelper = ({
    ...
    if (!device.firmwareReleaseConfig) {
...
export const onCallFirmwareUpdate = async ({
    ...
    const intermediary = !params.binary && device.firmwareReleaseConfig.intermediary;
    ...
    console.log('sending event FIRMWARE_DOWNLOADED');
    ...
    if (deviceInitiallyConnectedInBootloader) {
        ...
        const languageBlob =
            device.firmwareReleaseConfig && language && targetLanguage !== 'en-US'
                ? await getLanguage({
                      language: targetLanguage,
                      version: device.firmwareReleaseConfig.release.version,
                ...
                : null;
        console.log('languageBlob', languageBlob);
