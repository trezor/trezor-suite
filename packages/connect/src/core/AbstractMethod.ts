...
export abstract class AbstractMethod<P extends CallMethodPayload['method']> {
    responseID: number;

    // REF-TODO: strictPropertyInitialization
    device: Device;
    ...
    getButtonRequestData?(code: string): UiRequestButtonData | void;

    // callbacks
    // REF-TODO: strictPropertyInitialization1
    postMessage: (message: CoreMessage) => void;
    // REF-TODO
    getPopupPromise: () => Deferred<void>;
    // REF-TODO
    createUiPromise: (promiseId: any, device?: any) => any;
    // REF-TODO
    removeUiPromise: (promise: Deferred<UiPromiseResponse>) => void;

    constructor(message: { id?: number; payload: Payload<P> }) {
        const { payload } = message;
        this.name = payload.method;
        this.payload = payload;
        this.responseID = message.id || 0;
        this.devicePath = payload.device?.path;
        // REF-TODO: this is buggish, it allows undefined to be set, but we probably use this
        // as keys in metadata
        // @ts-expect-error
    ...
    abstract run(): Promise<MethodReturnType<P>>;
