...
describe('utils/urlUtils', () => {
    it('getOrigin', () => {
        ...
        expect(utils.getOrigin('https://foo.connect.trezor.io/8/?query=1#hash2')).toEqual(
    ...
    it('getOnionDomain', () => {
        ...
        expect(
            utils.getOnionDomain('http://foo.bar.connect.trezor.io/8/?query=1#hash2', dict),
        ).toEqual('http://foo.bar.connect.trezor.onion/8/?query=1#hash2');
