...
// Re-export wire types from blockchain-link-types/src/blockfrost.ts
...
export interface CardanoStakingInfo {
    address: string;
    isActive: boolean;
    rewards: string;
    poolId: string | null;
...
export interface CardanoMisc {
    rewards?: string;
    staking?: CardanoStakingInfo;
}

export type CardanoTransaction = Transaction<{
    subtype?:
        | 'withdrawal'
        | 'stake_delegation'
        | 'stake_registration'
...
export type CardanoAccountInfo = AccountInfo<CardanoMisc>;

export type CardanoUtxo = Utxo & { cardanoSpecific: { unit: string } };
