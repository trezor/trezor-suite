...
module.exports.patchBinaries = async () => {
    const binResourcesPath = path
        .join(__dirname, '../../../..', '/trezor-suite/node_modules/electron/dist/resources/bin/')
        .toString();

    const trezordPathFrom = path.join(
        __dirname,
        '../../../..',
        'suite-data/files/bin/bridge/linux-x64/trezord',
    );
    const trezordPathTo = path.join(binResourcesPath, 'bridge/linux-x64');
    ...
    const torPathFrom = path.join(
        __dirname,
        '../../../..',
        'suite-data/files/bin/tor/linux-x64/tor',
    );
    const torPathTo = path.join(
        __dirname,
        '../../../../..',
        '/node_modules/electron/dist/resources/bin/tor',
    );
