...
describe(compareMetric.name, () => {
    it('passes a metric that stays within its limit', () => {
        ...
        expect(comparison).toMatchObject({ exceededLimit: false, failed: false, limit: 1000 });
    });
    ...
    it('fails a metric that goes over its limit', () => {
        ...
        expect(comparison).toMatchObject({ exceededLimit: true, failed: true });
    });

    // The whole point of the limits: what the app used to cost has no say in what it may cost.
    it('fails a metric over its limit even when it is at the baseline', () => {
    ...
    it('passes a metric within its limit however far above the baseline it is', () => {
        ...
        expect(comparison).toMatchObject({ failed: false, ratioToBaseline: 9 });
    });
    ...
    it('never gates a metric the scenario sets no limit for', () => {
        const comparison = compareMetric(blockingTime, 100, undefined, 100_000);

        expect(comparison).toMatchObject({ limit: null, ratioToLimit: null, failed: false });
    });

    it('never gates a metric that could not be measured', () => {
        const comparison = compareMetric(blockingTime, 100, 1000, null);

        expect(comparison).toMatchObject({ current: null, ratioToLimit: null, failed: false });
    });
    ...
    it('keeps the baseline for the report even though it does not gate', () => {
        ...
        expect(comparison).toMatchObject({ baseline: 400, ratioToBaseline: 2 });
    });
});
...
describe(compareScenario.name, () => {
    ...
    it('fails the scenario when any metric goes over its limit', () => {
        ...
        expect(comparison.failed).toBe(true);
        expect(
            comparison.metrics.filter(metric => metric.failed).map(metric => metric.key),
        ).toEqual(['reactCommitCount']);
    });

    it('passes the scenario when every limited metric is within its limit', () => {
        const comparison = compareScenario('wallet-discovery', metrics(), undefined, limits);

        expect(comparison.failed).toBe(false);
    });

    it('reports a scenario nothing is limited for, so that it cannot look enforced', () => {
        const comparison = compareScenario('wallet-discovery', metrics(), undefined, undefined);

        expect(comparison).toMatchObject({ failed: false, unlimited: true });
    });
    ...
    it('treats a null limit the same as an absent one', () => {
        ...
        expect(comparison.metrics[0]).toMatchObject({ limit: null, failed: false });
    });
    ...
    it('carries the recorded baseline into the comparison', () => {
        ...
        expect(comparison.metrics[0]).toMatchObject({ baseline: 50, ratioToBaseline: 2 });
    });
});
