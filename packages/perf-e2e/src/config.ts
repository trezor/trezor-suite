...
export const getMetricDefinition = (key: PerfMetricKey): MetricDefinition => {
    const definition = METRIC_DEFINITIONS.find(def => def.key === key);
    if (!definition) {
        throw new Error(`Unknown performance metric: ${key}`);
    }
