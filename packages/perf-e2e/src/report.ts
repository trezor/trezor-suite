...
const formatMetricBlock = (metric: MetricComparison): string =>
    [
        ...
        `baseline: ${formatValue(metric.baseline, metric.unit)} (${formatRatio(
            metric.ratioToBaseline,
        )}, not enforced)`,
    ].join('\n');
...
export const formatHumanReport = (comparison: ScenarioComparison): string => {
    ...
    const failedMetrics = comparison.metrics.filter(metric => metric.failed);
    const body = failedMetrics.map(formatMetricBlock).join('\n\n');
...
export const buildJsonReport = (comparison: ScenarioComparison): PerfJsonReport => ({
    ...
    metrics: comparison.metrics.map(metric => ({
        ...
        limit: metric.limit,
        ratioToBaseline: metric.ratioToBaseline,
