...
const joinInputsCredentials = async (
    ...
    options.log(`Joining Credentials for account <${accountKey}>. Total inputs: ${inputs.length}`);
    ...
    for (let i = 1; i < inputs.length; i++) {
        ...
        if (current.error) {
            options.log(`Trying to join input with error. <${current.outpoint}> ${current.error}`);
            throw current.error;
        }
        options.log(
