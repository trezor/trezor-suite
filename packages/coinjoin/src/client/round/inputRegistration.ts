...
const registerInput = async (
    ...
    // setup random delay for registration request. note that this may cause that the input will not be registered if phase change before expected deadline
    const deadline = round.phaseDeadline - Date.now() - 10000; // TODO: contstant
    const delay = getRandomNumberInRange(0, deadline);
