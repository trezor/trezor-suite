/**
 * HTTP handlers for /log (receive analytics) and /api/analytics-events (read/SSE/clear).
 * Sets CORS so that Suite (web/desktop) on another origin can send events to /log.
 */
...
/**
 * Handle GET /log?c_type=...&... - receive analytics event from Suite (same as data.trezor.io).
 */
...
/**
 * Handle GET /api/analytics-events - return all events as JSON.
 */
...
/**
 * Handle GET /api/analytics-events/clear - clear events (query param ?clear=1 or POST for safety).
 */
...
/**
 * Handle GET /api/analytics-events/stream - Server-Sent Events for live updates.
 */
