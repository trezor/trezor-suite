...
echo "sync "$SOURCE" with "$BUCKET"/wallet/static"
...
if [ "x$3" == "x-clear" ]; then
    aws s3 sync --delete --cache-control 'public, max-age=3600' $SOURCE s3://$BUCKET/wallet/static/desktop
else
    aws s3 sync --cache-control 'public, max-age=3600' $SOURCE s3://$BUCKET/wallet/static/desktop
fi

aws cloudfront create-invalidation --distribution-id $DISTRIBUTION_ID --paths '/wallet/static/desktop/*'
