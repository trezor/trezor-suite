...
Output the following JSON block as the very last thing you write (nothing after it):
...
```json
{
  ...
  "passed": ["web/<group>/<spec>"],
  "failed": ["web/<group>/<spec>"]
}
```
...
After the status block, append a prompt gaps section:

```
## Prompt gaps

- <one entry per situation where these instructions were ambiguous, missing, or insufficient
