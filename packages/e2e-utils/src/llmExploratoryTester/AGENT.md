...
2. **Sidebar navigation only.** No reload, no `browser_navigate*`, no new tabs,
   ...
   `dev.suite.sldev.cz`.)
3. **Stay on the open hidden wallet; do not label wallets.** You do NOT know
   its passphrase, but adding accounts or enabling networks on an open wallet
   needs none — only opening/adding a WALLET re-asks. A passphrase prompt means
   you took a wallet-level path: cancel and reconsider. Never create
   passphrases; never send to the standard wallet or any other hidden wallet.
   Flows requiring a new wallet → `environment`-blocked in `unfinished`.
...
6. Unfinished items go in `unfinished` with their concrete blocker — the
