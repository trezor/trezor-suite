...
const ALLOWED_TOOLS = new Set([
    ...
    // Coordinate tools are registered only with playwright MCP `--caps=vision`.
...
export const sandboxGate = () => ({
    'tool.execute.before': (input, output) => {
        ...
        if (toolName.endsWith('browser_take_screenshot')) {
            // Filename omitted: Playwright attaches the image to the model.
            // Filename present: evidence file, must stay in the reports dir.
