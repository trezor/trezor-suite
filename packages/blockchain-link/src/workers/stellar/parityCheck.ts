 ...
 * real descriptors on the same origin. This is the check that validated the migration.
 *
 *   yarn workspace @trezor/blockchain-link stellar:parity <descriptor> [<descriptor> …]
 *
 * Include an account with non-curated trustlines to quantify the allow-list regression: the summary
 * reports which trustlines an RPC-only discovery would not have found.
 */

const DEFAULT_URL = process.env.STELLAR_URL ?? 'https://xlm.trezor.io';
