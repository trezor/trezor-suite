...
export const getInfo = async (request: Request<MessageTypes.GetInfo>, isTestnet: boolean) => {
    ...
    // Read through the data source: this is the handshake every other request waits on, so it is
    // the one that must survive an RPC outage by degrading to Horizon.
