...
export const subscribeBlock = async ({ state, connect, post }: Context) => {
    ...
    // Through the data source, so the poll reads the head from whichever backend serves it cheaply:
    // `getLatestLedger` ships the whole ledger close meta, megabytes an hour at this interval.
