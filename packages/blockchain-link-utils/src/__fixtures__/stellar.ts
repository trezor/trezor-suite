...
// nested `TOKEN_CONTRACT.transfer(DESCRIPTOR, ROUTER_CONTRACT, 20000000)`. A contract token is not
// a SAC, so Horizon reports no balance change and the envelope is the only record of what moved.
...
export const fixtures: { transformTransaction: Fixture[] } = {
    transformTransaction: [
        ...
        {
            // The mirror of the case above: the lumens arrive, so they cannot be an amount with a
            // target, which the shared shape reads as "sent".
        ...
        {
            // Both legs are lumens, so the account only moved their difference.
