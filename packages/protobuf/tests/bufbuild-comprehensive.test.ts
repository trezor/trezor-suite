...
const getAllProtoModules = () => {
    ...
    protoFiles.forEach(name => {
        try {
            ...
            console.warn(`Failed to load proto module: ${name}`);
        }
    });
...
describe('ProtobufManager comprehensive tests', () => {
    ...
    // Helper to get all message names from messages.json
    const getAllMessageNames = (): string[] => Object.keys(messagesJsonSchema.nested ?? {});

    // Helper to get all enum names from messages.json
    ...
    // Helper to check if message exists in @bufbuild definitions
    ...
    // Helper to check if enum exists in @bufbuild definitions
    ...
    // Helper to determine if a field is required using RULE_PATCH and @bufbuild definitions
    ...
    // Helper to get required fields from a message definition using RULE_PATCH
    ...
    // Helper to build minimal test data based on required fields from @bufbuild definitions
    const buildMinimalTestData = (messageName: string): TestPayload | null => {
        try {
            ...
            // Fallback to messages.json if manager.findSchema fails
            const messageDef = getMessageDefinition(messageName);
            if (!messageDef?.fields) {
                return null;
            }
    ...
    // Cross-check: Log which messages and enums are available and missing in @bufbuild
    describe('message and enum availability', () => {
        it('should identify all available and missing messages and enums in @bufbuild definitions', () => {
    ...
    // Test all available messages with JSON composition
    describe('Messages - JSON round-trip', () => {
        const allMessages = getAllMessageNames().filter(name => messageExistsInBufbuild(name));

        // Sample subset for test execution
        const samplesToTest = allMessages; // .slice(0, 20);
    ...
    // Test message composition patterns
    describe('4. Message Composition - Nested and Repeated', () => {
        ...
        compositionTests.forEach(({ description, messageName, data }) => {
            ...
            it(description, () => {
                try {
                    ...
                    // Some messages may not have these exact fields - that's ok
                    console.warn(
                        `${description}: ${getErrorMessage(error)} (expected for some messages)`,
                    );
                }
            });
        });
    ...
    // Test enum availability and usage
    describe('Enum Availability and Usage', () => {
        const criticalEnums = [
            'MessageType',
            'ThpMessageType',
            'InputScriptType',
        ...
        criticalEnums.forEach(enumName => {
            it(`${enumName} - should exist in @bufbuild definitions`, () => {
                ...
                if (exists) {
                    ...
                    console.error(`${enumName} not available in @bufbuild`);
                }
            });
        });
    });
