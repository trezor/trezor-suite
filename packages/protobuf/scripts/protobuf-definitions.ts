...
const modifyDefinitionsJSON = (root: protobuf.Root, def: Definition) => {
    ...
    if (def.options) {
        ...
        const options = typedObjectKeys(def.options);
        ...
        if (typedObjectKeys(opts).length < 1) {
            delete def.options;
        }
    }
    ...
    const { nested } = def;
    if (nested) {
        typedObjectKeys(nested).forEach(key => {
            const item = nested[key];
            const isInternal =
                item.options && typedObjectKeys(item.options).includes('(internal_only)');
...
const modifyMessageType = (proto: protobuf.Root, name: string) => {
    ...
    if (messageTypeEnum) {
        typedObjectKeys(messageTypeEnum.values).forEach(key => {
...
export const buildDefinitions = (protoDir: string, args: BuildOptions) => {
    ...
    typedObjectKeys(topLevelMessages).forEach(name => {
