...
export const init: Module = ({ mainWindow, store }) => {
    ...
    let feedURL = updaterURL || allowPrerelease ? defaultFeedURL.preRelease : defaultFeedURL.latest;

    autoUpdater.logger = null;

    if (updaterURL) {
        autoUpdater.setFeedURL(updaterURL);
        logger.warn(SERVICE_NAME, [`Feed url: ${updaterURL}`]);
    }
    ...
    ipcMain.on('update/allow-prerelease', (_, value = true) => {
        ...
        feedURL = allowPrerelease ? defaultFeedURL.preRelease : defaultFeedURL.latest;
