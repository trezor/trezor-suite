...
test.describe('Trading - Swap Solana', { tag: ['@group=other', '@webOnly'] }, () => {
    ...
    test('Swap Solana to Bitcoin', async ({ marketPage, page, tradingMock, devicePrompt }) => {
        await test.step('Fill in a Swap form', async () => {
            await marketPage.setYouSwapAmount(sendAmount, 'solana', 'SOL');
            await expect(marketPage.swapToCryptoInput).toContainText('Bitcoin');
        });
