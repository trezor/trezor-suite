...
import { createSuiteTauriCompositionRoot } from './createSuiteTauriCompositionRoot';
...
export const init = async (container: HTMLElement) => {
    ...
    const { store, services } = createSuiteTauriCompositionRoot(preloadAction, statePatch);
    ...
    // NOTE: unlike the Electron build, connect Core is NOT reached over an IPC proxy to a Node
    // main process. TrezorConnect (via the connect-web alias) initialises lazily inside the
    // webview through connectInitThunk, talking to the local Bridge — so there is no
    // `TrezorConnect.initIpcProxy()` here.

    // init bluetooth (BLE) transport. The BLE stack runs in the bluetooth-host Node sidecar; the
    // renderer reaches it over `window.ipcProxy` (Tauri preload → ws://127.0.0.1:21329), the same
    // @trezor/ipc-proxy path the Electron build uses. With no previously-paired devices this is a
    // no-op that only registers listeners (BluetoothIpc.init returns success without touching the
    // adapter), so it is safe on machines without BLE.
