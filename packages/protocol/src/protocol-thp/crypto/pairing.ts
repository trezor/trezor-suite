...
export const validateHP5 = (
    ...
    if (calculatedValue !== BigInt(value)) {
        throw new Error(`validateHP5: code don't match ${value} != ${calculatedValue.toString()}`);
    }
};
...
export const validateHP6 = (
    ...
    if (calculatedValue !== value) {
        throw new Error(`validateHP6: code don't match ${value} != ${calculatedValue}: secret`);
    }
};
...
export const validateHP7 = (
    ...
    if (Buffer.compare(calculatedValue, expectedValue)) {
        throw new Error(
            `validateHP7: code don't match ${calculatedValue.toString('hex')} != ${expectedValue.toString('hex')}: secret`,
        );
    }
};
