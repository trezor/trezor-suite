import { decode, encode, encodeAck } from '../src/protocol-thp';
...
describe('protocol-thp', () => {
    ...
    const thpState: any = {
        ...
        handshakeCredentials: {
            handshakeHash: Buffer.from('aa'),
            hostKey: Buffer.from('bb'),
        },
    };
    ...
    it('encode/decode ThpCreateChannelRequest', () => {
        ...
        expect(decoded.type).toEqual('ThpCreateChannelRequest');
        // expect(decoded.payload).toEqual(nonce);
    });

    it.skip('decode ThpCreateChannelResponse', () => {
        ...
        // @ts-expect-error
        expect(decoded.message.mockRawData).toEqual(devicePropertiesProtobuf);
    });
    ...
    it('encode/decode ThpReadAck', () => {
        ...
        const encodeAsState1 = encodeAck(thpState);
        expect(encodeAsState1.toString('hex')).toEqual('2012340004d9fcce58');
        const encodeAsState2 = encodeAck({ ...thpState, recvBit: 1 });
        expect(encodeAsState2.toString('hex')).toEqual('2812340004e98c8599');

        expect(decode(decodeV2(encodeAsBytes1), protobufDecoder).type).toBe('ThpReadAck');
        expect(decode(decodeV2(encodeAsBytes2), protobufDecoder).type).toBe('ThpReadAck');
        expect(decode(decodeV2(encodeAsState1), protobufDecoder).type).toBe('ThpReadAck');
        expect(decode(decodeV2(encodeAsState2), protobufDecoder).type).toBe('ThpReadAck');
    });
    ...
    it('encode/decode ThpHandshakeInitRequest', () => {
        ...
        const decoded = decode(decodeV2(encoded), protobufDecoder);
        expect(decoded.type).toEqual('ThpHandshakeInitRequest');
        // key + thpState.tag + crc
    ...
    it('encode/decode ThpHandshakeCompletionRequest', () => {
        ...
        const encoded = encode({
            ...
            protobufEncoder: () => ({
                messageType: 0,
                message: Buffer.from('29292929', 'hex'),
            }),
            thpState: { ...thpState, sendBit: 1 },
        });
        expect(encoded.toString('hex')).toEqual(
        ...
        const decoded = decode(decodeV2(encoded), protobufDecoder);
        expect(decoded.type).toEqual('ThpHandshakeCompletionRequest');
        // // key + thpState.tag + crc
        // expect(decoded.payload.toString('hex')).toEqual(
        //     '07070707070707070707070707070707a0a1a2a3a4a5a6a7a8a9b0b1b2b3b4b5d47b551c',
        // );
