...
function (_AbstractMethod) {
  ...
  function CardanoGetAddress(message) {
    ...
    _this.firmwareRange = (0, _paramsValidator.getFirmwareRange)(_this.name, (0, _CoinInfo.getMiscNetwork)('Cardano'), _this.firmwareRange); // create a bundle with only one batch if bundle doesn't exists
    ...
    if (bundle.length === 1) {
      _this.info = "Export Cardano address for account #" + ((0, _pathUtils.fromHardened)(_this.params[0].path[2]) + 1);
    } else {
