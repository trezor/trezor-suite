...
import getAccountInfo from '../../../data/methods/common/getAccountInfo';
...
export const UsingPathSchema = Type.Object({
    ...
    coin: Type.String({
        description:
            'determines the network definition. Coin `shortcut` (a `CoinSymbol`, e.g. `btc`) is used — see [supported coins](/details/coins) for every accepted value.',
    }),
});
...
export const GetAccountInfoSchema = Type.Intersect([
    Type.Union([UsingPathSchema, UsingPubkeySchema]),
    OtherParamsSchema,
]);
...
export const paramDescriptions = {
    ...
    defaultAccountType:
        'Deprecated, no-op: this parameter only affected the account-selection popup of the removed on-device discovery flow and is now ignored by `getAccountInfo`',
