...
} from '@trezor/connect-common/src/types/api/ethereum';
...
export const paramDescriptions = {
    ...
    data: 'type of [`EthereumSignTypedDataMessage`](https://github.com/trezor/trezor-suite/blob/develop/packages/connect-common/src/types/api/ethereum/index.ts)`. A JSON Schema definition can be found in the [EIP-712 spec](https://eips.ethereum.org/EIPS/eip-712).',
    metamask_v4_compat:
...
// `domain_separator_hash` and `message_hash` are computed internally when the
// device requires them (T1B1 firmware). Pass `data` directly for all models.
...
[EthereumMessageSignature type](https://github.com/trezor/trezor-suite/blob/develop/packages/protobuf/src/definitions/messages-ethereum.ts)
