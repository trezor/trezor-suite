...
export const paramDescriptions = {
    ...
    coin: 'determines the network definition. Coin `shortcut` (a `CoinSymbol`, e.g. `btc`) is used — see [supported coins](/details/coins) for every accepted value. If `coin` is not set API will try to get network definition from `path`.',
