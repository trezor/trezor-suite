import { PushTransaction } from '@trezor/connect-common/src/types/api/pushTransaction';

import { ApiPlayground } from '../../../components/ApiPlayground';
import { CommonParamsLink } from '../../../components/CommonParamsLink';
import { ParamsTable } from '../../../components/ParamsTable';
import pushTransaction from '../../../data/methods/bitcoin/pushTransaction.ts';

<ApiPlayground
    options={[
        { title: 'Push transaction', legacyConfig: pushTransaction[0] },
        { title: 'Advanced schema', method: 'pushTransaction', schema: PushTransaction },
    ]}
/>

export const paramDescriptions = {
    tx: 'serialized transaction,',
    coin: 'Determines network definition specified in [coins.json](https://github.com/trezor/trezor-suite/blob/develop/packages/connect-data/files/coins.json) file. Coin `shortcut`, `name` or `label` can be used.',
};

## Push transaction

Broadcast the transaction to the selected network.

```javascript
const result = await TrezorConnect.pushTransaction(params);
```

### Params

<CommonParamsLink />

#### PushTransaction

<ParamsTable schema={PushTransaction} descriptions={paramDescriptions} />

### Example

```javascript
TrezorConnect.pushTransaction({
    tx: '',
    coin: 'btc',
});
```

### Result

[PushedTransaction type](https://github.com/trezor/trezor-suite/blob/develop/packages/connect-common/src/types/api/pushTransaction.ts)

```javascript
{
    success: true,
    payload: {
        txid: string, // transaction id
    }
}
```

Error

```javascript
{
    success: false,
    error: {
        message: string // error message
    }
}
```
