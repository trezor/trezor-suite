...
Work through the breaking sections that apply to you, adopt the recommended changes if you want them, then verify. For the architecture behind the change, see [New Connect flow in Trezor Suite](/guides/new-connect-flow-in-trezor-suite); for the release-by-release list, see the [changelog](/changelog).
...
Most integrations only touch a few of these. Find your call site or your error in the table, then jump to the section that explains it.

| v9 API                            | v10                              | What to do                                                                                                                  |
| --------------------------------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `getAccountDescriptor`            | Removed                          | Call the per-coin `*GetPublicKey`, read `displayablePublicKey` ([these APIs](#breaking-changes-only-if-you-use-these-apis)) |
| `getAccountInfo()` with no path   | Throws `Method_InvalidParameter` | Pass `path` or `descriptor`, or use `selectAccount` ([these APIs](#breaking-changes-only-if-you-use-these-apis))            |
| `coin: 'Bitcoin'` (name/label)    | Rejected                         | Use the shortcut: `coin: 'btc'` ([these APIs](#breaking-changes-only-if-you-use-these-apis))                                |
| `useCardanoDerivation`            | Removed                          | Declare `enabledNetworks` at init ([Cardano](#cardano))                                                                     |
| `on` / `off` / `uiResponse`       | Throws `TypeError`               | Remove — no replacement in the public tier ([these APIs](#breaking-changes-only-if-you-use-these-apis))                     |
| device-management methods         | Not in public API                | Suite's responsibility now — no replacement ([these APIs](#breaking-changes-only-if-you-use-these-apis))                    |
| `@trezor/connect-plugin-ethereum` | Stub that throws                 | Drop it, remove `transformTypedData` ([these APIs](#breaking-changes-only-if-you-use-these-apis))                           |
| `disableWebUSB()`                 | Removed                          | `updateConnectSettings({ transports })` ([Core hosts](#breaking-changes-only-if-you-host-connect-core-trezorconnect))       |

## Breaking changes: every integration
...
## Breaking changes: only if you use these APIs
...
## Breaking changes: only if you host Connect Core (`@trezor/connect`)

Skip this section if you install `@trezor/connect-web`, `@trezor/connect-webextension` or `@trezor/connect-mobile` — those are thin clients and Suite hosts the core for them.
...
Nothing here breaks your integration, but these are the improvements Connect 10 was built for.
