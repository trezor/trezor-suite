...
This guide is the single source of truth for the upgrade. It is edited as the betas land. For the architecture behind the change, see [New Connect flow in Trezor Suite](/guides/new-connect-flow-in-trezor-suite). For the release-by-release list, see the [changelog](/changelog).
...
- **Steps 1, 2 and 5 are the breaking changes** — your integration will not work correctly until you have done the ones that apply to you. Step 5 applies only if you host Connect Core (`@trezor/connect`) yourself; if you install `@trezor/connect-web`, `-webextension` or `-mobile`, skip it.
- **Step 3 is optional** — improvements worth adopting, but nothing breaks if you do not.
- **Step 4 is a verification pass** once you are done.
...
| --------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `getAccountDescriptor`            | Removed                          | Call the per-coin `*GetPublicKey`, read `displayablePublicKey` ([Step 2](#step-2-breaking--only-if-you-use-these-apis)) |
| `getAccountInfo()` with no path   | Throws `Method_InvalidParameter` | Pass `path` or `descriptor`, or use `selectAccount` ([Step 2](#step-2-breaking--only-if-you-use-these-apis))            |
| `coin: 'Bitcoin'` (name/label)    | Rejected                         | Use the shortcut: `coin: 'btc'` ([Step 2](#step-2-breaking--only-if-you-use-these-apis))                                |
| `useCardanoDerivation`            | Removed                          | Declare `enabledNetworks` at init ([Cardano](#cardano))                                                                 |
| `on` / `off` / `uiResponse`       | Throws `TypeError`               | Remove — no replacement in the public tier ([Step 2](#step-2-breaking--only-if-you-use-these-apis))                     |
| device-management methods         | Not in public API                | Suite's responsibility now — no replacement ([Step 2](#step-2-breaking--only-if-you-use-these-apis))                    |
| `@trezor/connect-plugin-ethereum` | Stub that throws                 | Drop it, remove `transformTypedData` ([Step 2](#step-2-breaking--only-if-you-use-these-apis))                           |
| `disableWebUSB()`                 | Removed                          | `updateConnectSettings({ transports })` ([Step 5](#step-5-breaking--only-if-you-host-connect-core-trezorconnect))       |

## Step 1: Breaking — required for every integration
...
## Step 2: Breaking — only if you use these APIs
...
## Step 3: Optional — recommended, not required
...
## Step 5: Breaking — only if you host Connect Core (`@trezor/connect`)
