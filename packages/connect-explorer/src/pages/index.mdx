...
import { Button, CollapsibleBox, Row } from '@trezor/components';
...
<BetaOnly>
    <SectionCard>
        ...
        A new major version, currently in beta. The integration is now built around **Trezor
        Suite**: Suite hosts the Connect core and runs the device flow, so users get a better
        experience and you get a cleaner API. Parts of your integration change — the migration
        checklist covers them.

        Already integrated with v9? It keeps working: v9 stays supported and its docs stay online,
        so you can move over whenever it suits you.

        <Row gap={12} flexWrap="wrap" margin={{ top: 16 }}>
            <LinkButton intent="brand" size="small" href="/guides/migrating-to-connect-10">
                Migration checklist
            </LinkButton>
            <Button
                intent="neutral"
                priority="secondary"
                size="small"
                href="https://connect.trezor.io/9/"
                target="_blank">
                Connect 9 docs
            </Button>
        </Row>
...
<SectionCard>
    ...
    <CollapsibleBox heading="Still unsure?" margin={{ top: 16 }}>
        |Environment|Package|
        |---|-------|
        |node|@trezor/connect|
...
<SectionCard>
    ...
    <SdkContainer>
        ...
        <ExamplesAside>
            ...
            - [Trezor Suite (desktop) electron app](https://github.com/trezor/trezor-suite/blob/develop/suite/desktop-app/README.md)
        </ExamplesAside>
    </SdkContainer>

    <CollapsibleBox heading="Quick start manual" margin={{ top: 16 }}>
        <Steps>
...
<SectionCard>
    ...
    <Tabs items={['Suite desktop popup', 'Suite Web popup']}>
        <Tabs.Tab>
            <ZoomableIllustration
                src="/images/schema-connect-desktop.svg"
                $darkMode
                alt="connect schema when used with Suite Desktop popup"
            />
        </Tabs.Tab>
        <Tabs.Tab>
            <ZoomableIllustration
