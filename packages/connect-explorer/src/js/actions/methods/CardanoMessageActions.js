...
const PREFIX: string = 'cardano_signverify';
...
export function onSignMessage(tx: string): any {
    return async function (dispatch, getState) {
        const response = await TrezorConnect.cardanoSignMessage( getState().common.params );
...
export function onVerifyMessage(tx: string): any {
    return async function (dispatch, getState) {
        const response = await TrezorConnect.cardanoVerifyMessage( getState().common.params );
        dispatch( onResponse(response) );
    }
}
...
export function verifyResponseValues(response: any): any {
    return function (dispatch, getState) {
        ...
        dispatch( push('cardano-verifymessage') );
    }
}