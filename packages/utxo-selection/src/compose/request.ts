...
function validateAndParseUtxos(
    ...
    for (let i = 0; i < utxos.length; i++) {
        try {
            ...
            return incorrectUtxoError(i, (error as Error).message);
        }
    }
...
function transformOutput(
    ...
    txType: CoinSelectPaymentType,
    convertAddress: Request['convertAddress'],
    convertOpReturn?: Request['convertOpReturn'],
): CoinSelectOutput {
    const script = { length: OUTPUT_SCRIPT_LENGTH[txType] };
    if (output.type === 'payment') {
        const value = bignumberOrNaN(output.amount);
        if (value === undefined) throw new Error('Invalid amount');

        return {
            value,
            script: convertAddress(output.address),
        };
    }
    ...
    if (output.type === 'opreturn') {
        if (!convertOpReturn)
            throw new Error('convertOpReturn callback is required for opreturn outputs');

        return {
            value: bignumberOrNaN('0', true),
            script: convertOpReturn(output.dataHex),
        };
    }
...
function validateAndParseOutputs(
    txType: CoinSelectPaymentType,
    { outputs, convertAddress, convertOpReturn }: Request,
):
    | {
    ...
    for (let i = 0; i < outputs.length; i++) {
        ...
        try {
            const csOutput = transformOutput(output, txType, convertAddress, convertOpReturn);
            csOutput.weight = outputWeight(csOutput);
            result.push(csOutput);
        } catch (error) {
            return incorrectOutputError(i, (error as Error).message);
        }
    }
