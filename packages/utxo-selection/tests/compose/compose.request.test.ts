...
import type { ComposeRequest } from '../../src/compose/types';
...
const toOutputScript = (address: string) => {
    if (address === 'weird-address') throw new Error(`${address} has no matching Script`);
...
const REQUEST: ComposeRequest<any, any, any> = {
    ...
    toOutputScript,
    toOpReturnScript: dataHex => ({ length: 2 + dataHex.length / 2 }),
};
