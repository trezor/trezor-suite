/** @deprecated */
export const MAX_ONBOARDING_WIDTH = 1020; // old max width, kept for consistency@deprecated
export const MAX_CONTENT_WIDTH_NUMERIC = 1200;
export const MAX_CONTENT_WIDTH = `${MAX_CONTENT_WIDTH_NUMERIC}px`;
export const HORIZONTAL_LAYOUT_PADDINGS = '16px';
export const SUBPAGE_NAV_HEIGHT_NUMERIC = 59;
export const SUBPAGE_NAV_HEIGHT = `${SUBPAGE_NAV_HEIGHT_NUMERIC}px`;
export const HEADER_HEIGHT_NUMERIC = 64;
export const HEADER_HEIGHT = `${HEADER_HEIGHT_NUMERIC}px`;
export const MIN_CONTENT_WIDTH = 300;
