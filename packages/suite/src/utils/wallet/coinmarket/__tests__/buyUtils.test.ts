...
describe('coinmarket/buy utils', () => {
    ...
    it('createQuoteLink', () => {
        ...
        expect(createQuoteLink(QUOTE_REQUEST_FIAT, accountMock)).toStrictEqual(
            `${window.location.origin}/trade-redirect#offers/btc/normal/1/qf/CZ/EUR/10/BTC`,
        );
        // @ts-ignore
        expect(createQuoteLink(QUOTE_REQUEST_CRYPTO, accountMock)).toStrictEqual(
            `${window.location.origin}/trade-redirect#offers/btc/normal/1/qc/CZ/EUR/0.001/BTC`,
        );
    });
    ...
    it('createTxLink', () => {
        ...
        expect(createTxLink(MIN_MAX_QUOTES_OK[0], accountMock)).toStrictEqual(
            `${window.location.origin}/trade-redirect#detail/btc/normal/1/e709df77-ee9e-4d12-98c2-84004a19c546`,
        );
    });
