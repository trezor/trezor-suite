...
type TradingOffersModalSellProps = {
    onClose: () => void;
};

export const TradingOffersModalSell = ({ onClose }: TradingOffersModalSellProps) => {
    const { control, getValues, setValue } = useFormContext<TradingSellFormProps>();
    const paymentMethod = useWatch({ control, name: TRADING_FORM_PAYMENT_METHOD_SELECT });
    const quotes = useSelector(state => selectTradingSellOfferQuotes(state, paymentMethod?.value));
