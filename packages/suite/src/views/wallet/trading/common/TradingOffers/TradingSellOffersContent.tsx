...
import { selectTradingSellBestQuote, selectTradingSellQuotes } from '@suite-common/trading';
...
export const TradingSellOffersContent = () => {
    const quotes = useSelector(selectTradingSellQuotes);
    const bestRatedQuote = useSelector(selectTradingSellBestQuote);
        ...
        <>
            ...
            <Column gap={16} margin={{ top: 16 }}>
                {noOffers ? (
                    ...
                    <Column gap={16}>
                        <TradingUtilsTorWarning tradingType="sell" noOffer={false} />
                        {quotes.map(quote => (
                            <TradingOffersItem
                                key={quote?.orderId}
                                quote={quote}
