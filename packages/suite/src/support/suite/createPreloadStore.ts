...
export const createPreloadStore =
    ...
    async () => {
        ...
        try {
            // The runtime handlers dispatch storageError into Redux, but during startup the store is
            // not hydrated yet and the outcome must become the preload action itself, so that
            // hydration returns storageError instead of storageLoad. Route the same IndexedDB events
            // into this promise while the database opens, then hand the dispatching handlers back.
