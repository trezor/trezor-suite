...
// import * as SETTINGS from '@wallet-actions/constants/settingsConstants';
import { httpRequest } from '@wallet-utils/networkUtils';
// import * as buildUtils from '@suite-utils/build';
import * as storageUtils from '@wallet-utils/storageUtils';
// import * as WalletSettingsActions from '@wallet-actions/settingsActions';
// import { getAccountTokens } from '@wallet-utils/reducerUtils';
...
import { TokensCollection } from '@wallet-reducers/localStorageReducer';
...
const KEY_VERSION = `${STORAGE_PATH}version`;
...
const KEY_TOKENS = `${STORAGE_PATH}tokens`;
...
// const findAccounts = (devices: Array<TrezorDevice>, accounts: Account[]) =>
...
//     );
//     const accounts: Account[] = findAccounts(devices, getState().accounts);
//     const tokens: Token[] = findTokens(accounts, getState().tokens);
//     const pending: Transaction[] = findPendingTxs(accounts, getState().pending);
//     const discovery: Discovery[] = findDiscovery(devices, getState().discovery);
...
// export const update = (event: StorageEvent) => (dispatch: Dispatch) => {
...
export const loadJSON = () => async (dispatch: Dispatch) => {
    ...
    try {
        ...
        const tokens = await NETWORKS.reduce(
            async (
                ...
                if (network.tokens) {
                    const json = await httpRequest(resolveStaticPath(network.tokens), 'json');
...
const loadStorageData = () => (dispatch: Dispatch) => {
    ...
    const userTokens = storageUtils.get(TYPE, KEY_TOKENS);
    if (userTokens) {
        dispatch({
            type: TOKEN.FROM_STORAGE,
    ...
    //     if (!betaModal) {
    //         dispatch({
    //             type: SETTINGS.SHOW_BETA_DISCLAIMER,
    //             show: true,
    //         });
    //     }
    // }
    ...
    // if (localCurrency) {
    //     dispatch(WalletSettingsActions.setLocalCurrency(JSON.parse(localCurrency)));
    // }

    // const hideBalance: ?string = storageUtils.get(TYPE, KEY_HIDE_BALANCE);
    // if (hideBalance) {
    //     dispatch(WalletSettingsActions.setHideBalance(JSON.parse(hideBalance)));
    // }
};
