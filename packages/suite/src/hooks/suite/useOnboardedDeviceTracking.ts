 ...
 *
 * Arming happens in the CTA that starts onboarding, because that is the last moment the selection
 * is certainly the device the user meant. Arming the firmware ref is left to `firmwareUpdateThunk`,
 * from the device the firmware step hands it — which is this one, so both refs follow the same
