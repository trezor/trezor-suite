...
export const useCoinmarketExchangeForm = ({
    ...
    const fiatValues = useCoinmarketFiatValues({
        ...
        fiatCurrency: values.outputs?.[0]?.currency?.value as FiatCurrencyCode,
    });
    ...
    const isInitialDataLoading = !exchangeInfo?.exchangeList || !values?.outputs?.[0].address;
    const isFormLoading =
    ...
    const getQuotesRequest = useCallback(
        async (request: ExchangeTradeQuoteRequest, offLoading?: boolean) => {
            ...
            if (
                !request.send ||
                !request.receive ||
                !request.sendStringAmount ||
                request.sendStringAmount === '' ||
                Number(request.sendStringAmount) === 0
            ) {
                timer.stop();
    ...
    const handleChange = useCallback(
        async (offLoading?: boolean) => {
            ...
            if (quotesRequest) {
                const allQuotes = await getQuotesRequest(quotesRequest, offLoading);
    ...
    useEffect(() => {
        if (exchangeType === FORM_EXCHANGE_DEX && !dexQuotes?.length && filteredCexQuotes?.length) {
            setValue(FROM_EXCHANGE_TYPE, FORM_EXCHANGE_CEX);
        } else if (
            exchangeType === FORM_EXCHANGE_CEX &&
            !filteredCexQuotes?.length &&
            dexQuotes?.length
        ) {
            setValue(FROM_EXCHANGE_TYPE, FORM_EXCHANGE_DEX);
        } else if (
