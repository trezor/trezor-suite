...
const prepareStorageMiddleware =
    (getExtra: () => ExtraDependencies | null) => (api: MiddlewareAPI<Dispatch, AppState>) => {
        ...
        return (next: Dispatch) =>
            (action: SuiteAction | WalletAction): SuiteAction | WalletAction => {
                ...
                if (!dbInitialized) {
                    db.onBlocking = () =>
                        api.dispatch({ type: STORAGE.ERROR, payload: 'blocking' });
                ...
                if (
                    ...
                    if (isDeviceRemembered(device) && isAccountSuccessful(newAccount)) {
                        storageActions.saveAccounts(db, [newAccount]);
                        api.dispatch(storageActions.saveCoinjoinAccount(newAccount.key));
                    }
                }

                if (
                    isAnyOf(deviceActions.setDeviceState, deviceActions.addAuthorizedDevice)(action)
                ) {
                    ...
                    if (device && !device.useEmptyPassphrase && isDeviceRemembered(device)) {
                        ...
                        storageActions.saveAccounts(db, accounts);
                    }
                }

                if (accountsActions.removeAccount.match(action)) {
                    action.payload.forEach(
                        storageActions.removeAccountWithDependencies(db, api.getState),
                    );
                }
                ...
                if (isAnyOf(metadataActions.setAccountAdd)(action)) {
                    ...
                    if (isDeviceRemembered(device) && isAccountSuccessful(action.payload)) {
                        storageActions.saveAccounts(db, [action.payload]);
                    }
                }
                ...
                if (transactionsActions.resetTransaction.match(action)) {
                    ...
                    storageActions.removeAccountTransactions(db, account);
                    storageActions.removeAccountHistoricRates(db, account.key);
                }
                ...
                if (
                    ...
                    if (account) {
                        ...
                        if (isDeviceRemembered(device)) {
                            ...
                            if (historicRates) {
                                api.dispatch(
                                    storageActions.saveAccountHistoricRates(
                                        account.key,
                                        historicRates,
                                    ),
                                );
                ...
                if (explorerActions.setExplorer.match(action)) {
                    storageActions.saveExplorer(db, action.payload);
                }
                ...
                if (deviceActions.updateSelectedDevice.match(action)) {
                    ...
                    if (
                        ...
                        (storageActions.saveAccounts(db, []) ?? Promise.resolve())
                            ...
                            .then(() => storageActions.saveDevice(db, action.payload));
                    }
                }
                ...
                switch (action.type) {
                    ...
                    case GRAPH.ACCOUNT_GRAPH_FAIL: {
                        ...
                        if (isDeviceRemembered(device)) {
                            storageActions.saveGraph(db, [action.payload]);
                        }
                        break;
                    }
                    case tradingActions.saveTrade.type: {
                        const { type, ...trade } = action;
                        storageActions.saveTradingTrade(db, trade.payload);
                        break;
                    }
...
export default prepareStorageMiddleware;
