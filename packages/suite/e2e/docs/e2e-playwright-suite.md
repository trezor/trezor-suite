...
1. In a third window, run `yarn workspace @trezor/suite test:e2e:web`.
...
1. `yarn workspace @trezor/suite test:e2e:desktop`

#### Troubleshooting

1. **To run tests headed (showing UI)** you can add: `--headed`.

1. **To run just one test file** you can do: `yarn workspace @trezor/suite test:e2e general/wallet-discovery.test.ts`
...
1. **To run with x-main firmware** instead of x-latest you can use CANARY_FIRMWARE env variable like this: `CANARY_FIRMWARE=true yarn workspace @trezor/suite test:e2e:web` or `CANARY_FIRMWARE=true yarn workspace @trezor/suite test:e2e:desktop`
...
1. Review all new image snapshots that were generated by the first step. They are automatically stored in `packages/suite/e2e/snapshots` folder and should be visible in your `git status`.
...
Traces contain electron logs of our desktop suit app. Both in CI, currents and local env (`packages/suite/e2e/test-results`).
