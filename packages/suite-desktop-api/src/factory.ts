...
export const factory = <R extends StrictIpcRenderer<any, IpcRendererEvent>>(
    ...
    changeTorSettings: payload => {
        if (validation.isObject({ useExternalTor: 'boolean', externalPort: 'number' }, payload)) {
    ...
    // bioAuth
    setBioAuthSettings: settings => ipcRenderer.invoke('bio-auth/set-bio-auth-settings', settings),
