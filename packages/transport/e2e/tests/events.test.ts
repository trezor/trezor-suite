...
describe('bridge', () => {
    ...
    beforeEach(async () => {
        ...
        bridge1 = new BridgeTransport({ messages, debug: false });
        bridge2 = new BridgeTransport({ messages, debug: false });
        ...
        descriptors = await bridge1.enumerate();
    ...
    test('2 clients. one acquires and releases, the other one is watching', async () => {
        ...
        const session1: string = await bridge1.acquire({ input: { previous: null, path: '1' } });
        ...
        await bridge1.release(session1);
        ...
        const session2 = await bridge2.acquire({ input: { previous: null, path: '1' } });
        expect(session2).toEqual('2');
    });
    ...
    test('session can be "stolen" by another client', async () => {
        ...
        const session2: string = await bridge2.acquire({
            input: { previous: session1, path: '1' },
        });
