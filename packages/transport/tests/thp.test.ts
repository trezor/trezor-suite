...
describe('thp', () => {
    ...
    describe('receiveThpMessage', () => {
        it('aborted', async () => {
            ...
            const apiRead2 = readWithAttempts(apiRead, { signal: abortController.signal });
        ...
        it('attempts limit reached', async () => {
            const result = await receiveThpMessage({
                thpState,
                apiRead: readWithAttempts(apiRead, { attempts: 20 }),
                apiWrite,
            });

            expect(apiRead).toHaveBeenCalledTimes(20);
            expect(result).toMatchObject({ success: false, message: 'Unexpected chunk' });
        });
        ...
        it('success. Expected chunk received at 5th attempt', async () => {
            ...
            const result = await receiveThpMessage({
                thpState,
                apiRead: readWithAttempts(apiRead),
                apiWrite,
            });
