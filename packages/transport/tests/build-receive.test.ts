...
import { bridge as bridgeProtocol, v1 as v1Protocol, v2 as v2Protocol } from '@trezor/protocol';
...
describe('encoding json -> protobuf -> json', () => {
    ...
    it('v1Protocol: unexpected chunk header', async () => {
    ...
    it.skip('Control bit test', () => {
        const setControlBit = (magic: number, cb: number) => magic | (cb << 4);
        const clearControlBit = (magic: number) => magic & ~(1 << 4);
        const getControlBit = (magic: number) => ((magic & (1 << 4)) === 0 ? 0 : 1);
    ...
    it('v2Protocol THP decode message', () => {
        ...
        payloads.forEach(async f => {
            const decodedResponse = await receiveAndParse(
                protobufRoot,
                () => Promise.resolve({ success: true, payload: Buffer.from(f.data, 'hex') }),
