...
import { Dot, type DotIntent } from './Dot';
...
export const ANIMATED_DOT_RINGING_DURATION = POP_DURATION + PULSE_DURATION * PULSE_ITERATIONS;
...
const RingingDot = styled.div<{ $size: number; $intent: DotIntent }>`
    position: relative;
    width: ${({ $size }) => $size}px;
    height: ${({ $size }) => $size}px;
    border-radius: 50%;
        ...
        {isShown && (
            ...
            >
                {isAnimated ? (
                    ...
                    <Dot size={size} intent={intent} data-testid={dataTestId} />
                )}
            </motion.div>
        )}
    </AnimatePresence>
