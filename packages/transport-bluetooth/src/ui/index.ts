...
const updateDeviceList = (api: TrezorBle, devices: BluetoothDevice[]) => {
    ...
    // const dupls = devices.reduce((prev, curr) => {
    //     const findByAddress = prev.findIndex(d => d.address === curr.address);
    //     if (findByAddress >= 0) {
    //         console.warn('Remove Duplicate!', prev[findByAddress]);
    //         prev.splice(findByAddress, 1);
    //     }
