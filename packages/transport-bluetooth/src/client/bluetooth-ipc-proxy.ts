...
/*
 * Shared class to be used in @trezor/suite-desktop
 * Provides ipc-proxy between main and renderer context
 * Usage:
 * - [main] just import and use
 * - [renderer] override
 */
...
export class BluetoothApiImpl extends TypedEmitter<BluetoothApiEvents> implements BluetoothIpcApi {
    ...
    private state: BluetoothIpcApiState = { knownDevices: [] };
    ...
    async setState(state: BluetoothIpcApiState) {
        this.state = state;
        if (state.knownDevices.length > 0) {
            try {
                ...
                const promises = devices.flatMap(d => {
                    ...
                    if (d.connected) {
                        return this.api.send('connect_device', d.uuid);
                    }
                ...
                console.warn('inital scan connected', devices);
            } catch (e) {
