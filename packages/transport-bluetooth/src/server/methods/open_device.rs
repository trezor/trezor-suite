...
pub async fn open_device(
    ...
    let device_1 = manager.get_device(id.clone()).await;
    if device_1.is_none() {
        Err("Device not found")?;
    }

    let adapter = adapter.unwrap();
    let device = utils::get_peripheral_by_address(&adapter, id.clone()).await?;
    // let device = utils::get_peripheral_by_address(&adapter, "id".to_string()).await?;
