...
async fn connect_device_inner(
    ...
    let dev = manager.get_device(id.clone()).await;
    if dev.is_none() {
    ...
    use std::{process::{Command, Child}, thread, time::Duration};
...
pub async fn connect_device(id: String, manager: AdapterManager) -> MethodResult {
    ...
    return connect_device_inner(id, adapter, manager).await;
}
...
async fn connect_device_common(
    ...
    let peripheral = utils::get_peripheral_by_address(&adapter, id.clone()).await?;
