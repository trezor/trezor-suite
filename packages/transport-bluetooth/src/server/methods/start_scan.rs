...
use crate::server::types::{
    AbortProcess, AdapterState, ChannelMessage, MethodResult, NotificationEvent, WsResponsePayload,
};
...
async fn scan(adapter: &Adapter) {
    ...
    {
        // - ScanFilter incorrectly work on windows https://github.com/deviceplug/btleplug/issues/249 (returns different device service uuid)
            ...
            match event {
                ChannelMessage::Abort(event) => {
                    if matches!(event, AbortProcess::Scan)
                        || matches!(event, AbortProcess::ClientDisconnected)
                    {
                ...
                ChannelMessage::Notification(event) => {
                    match event {
                        NotificationEvent::AdapterStateChanged { state } => {
                            match state {
