...
impl AdapterManager {
    ...
    async fn add_device(&self, id: String, device: TrezorDevice) {
        let devices = self.get_devices().await;
        let outdated = devices.into_iter().find(|d| d.get_address() == device.get_address() && d.get_id() != device.get_id());
            ...
            while let Some(event) = events.next().await {
                match event {
                    ...
                    CentralEvent::DeviceDiscovered(id) => {
                        ...
                        if evt.is_some() {
                            let peripheral = adapter.peripheral(&id).await.expect("Peripheral missing");
                            let device = TrezorDevice::new(peripheral).await.unwrap();
                            self_clone.add_device(id.to_string(), device.clone()).await;
