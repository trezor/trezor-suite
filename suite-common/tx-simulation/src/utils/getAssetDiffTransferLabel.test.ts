...
describe('getSolanaAssetDiffLabel', () => {
    it('converts the raw value with the asset decimals', () => {
        ...
        expect(getSolanaAssetDiffLabel(networkConfigDeps, diff, diff.out!, 'sol')).toBe(
            '0.004110331 SOL',
        );
    });
    ...
    it('keeps small amounts out of exponential notation', () => {
        ...
        expect(getSolanaAssetDiffLabel(networkConfigDeps, diff, diff.in!, 'sol')).toBe(
            '0.000001515 WSOL',
        );
    });
    ...
    it('falls back to the network symbol when the asset has none', () => {
        ...
        expect(getSolanaAssetDiffLabel(networkConfigDeps, diff, diff.in!, 'dsol')).toBe('1 dSOL');
    });
});
...
describe('getStellarAssetDiffLabel', () => {
    it('converts the stroop count instead of the rounded value', () => {
        ...
        expect(getStellarAssetDiffLabel(networkConfigDeps, diff, diff.out!, 'xlm')).toBe(
            '12.5000001 USDC',
        );
    });
    ...
    it('keeps amounts Blockaid rounds down to zero', () => {
        ...
        expect(getStellarAssetDiffLabel(networkConfigDeps, diff, diff.in!, 'xlm')).toBe(
            '0.0000004 yBTC',
        );
    });
    ...
    it('prefers the contract symbol', () => {
        ...
        expect(getStellarAssetDiffLabel(networkConfigDeps, diff, diff.in!, 'xlm')).toBe('3 AQUA');
    });
    ...
    it('falls back to the network symbol for the native asset without a code', () => {
        ...
        expect(getStellarAssetDiffLabel(networkConfigDeps, diff, diff.in!, 'xlm')).toBe('7 XLM');
    });
});
