...
import {
    ...
    compareEarnByNetwork as compareEarnByNetworkBase,
    compareEarnByNetworkTokenOrder as compareEarnByNetworkTokenOrderBase,
} from './earnSortUtils';

const compareEarnByNetwork = <T>(getSymbol: (item: T) => NetworkSymbol | undefined) =>
    compareEarnByNetworkBase(mockNetworkConfigDeps, getSymbol);

const compareEarnByNetworkTokenOrder = <T>(
    getKey: Parameters<typeof compareEarnByNetworkTokenOrderBase<T>>[1],
) => compareEarnByNetworkTokenOrderBase(mockNetworkConfigDeps, getKey);
