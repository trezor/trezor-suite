...
const iconsFilePath = '../../packages/components/src/components/Icon/types.ts';
...
const generateIconsFileContent = assetTypesArray => {
        ...
        ({ name, assets, dirname, typeName }) => `
           export type IconName = 
            ${assets
                .map(
                    ({ fileName }, index) =>
                        `'${fileName.replace('.svg', '')}' ${index === assets.length - 1 ? '' : '|'}`,
                )
                .join(' ')};
        ;
    ...
    console.log('mappedAssetTypes', mappedAssetTypes);
