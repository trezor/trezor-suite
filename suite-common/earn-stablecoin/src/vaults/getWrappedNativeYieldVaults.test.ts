...
describe('getWrappedNativeYieldVaults', () => {
    ...
    it('filters out vaults on other networks', () => {
        ...
        expect(
            getWrappedNativeYieldVaults({ vaults: [baseVault], networkSymbol: ethSymbol }),
        ).toEqual([]);
    });
    ...
    it('filters out vaults with deposits closed', () => {
        ...
        expect(
            getWrappedNativeYieldVaults({ vaults: [closedVault], networkSymbol: ethSymbol }),
        ).toEqual([]);
    });

    it('returns an empty array when vaults are not loaded', () => {
        expect(
            getWrappedNativeYieldVaults({ vaults: undefined, networkSymbol: ethSymbol }),
        ).toEqual([]);
    });
});
