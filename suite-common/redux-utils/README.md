...
| **secondary index** | A grouping index, one per entry in `secondaryIndexes`: a `Map` from a key the extractor returns to every entity that answers to it. Never unique — that is what the id is for. An extractor may return several keys, or `undefined` to leave the entity out of that index. |
