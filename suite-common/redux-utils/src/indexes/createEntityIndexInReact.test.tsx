...
// Nothing about the index needs a hook of its own: the index is a module-scope object and
// `getById` is `(state) => value`, so it is read from React the way every other selector is — a
// plain `useSelector` in a component, or inside a selector a component already calls.
describe('reading an entity by id through useSelector', () => {
    ...
    it('follows the id it is given when that changes', () => {
        ...
        const { result, rerender } = renderHook(
            ({ id }: { id: string }) => useSelector((state: State) => index.getById(state, id)),
            {
                wrapper: createWrapper(store),
                initialProps: { id: 'a' },
            },
...
describe('reading every id through useSelector', () => {
    it('reads every id', () => {
        const { index } = createIndex();
        const store = createTestStore([a, b]);

        const { result } = renderHook(() => useSelector((state: State) => index.getIds(state)), {
            wrapper: createWrapper(store),
        });
    ...
    it('hands back the same array while the entities are unchanged', () => {
        ...
        // invalidating them. This is the index's own array rather than one built per read, which
        // is what lets a plain `useSelector` return it without an equality function.
        const { index } = createIndex();
        const store = createTestStore([a, b]);
...
describe('the index a component is reading', () => {
    it('is built once for a whole list rendering on the same store state', () => {
        // The reason the index is one shared object in module scope: a hundred rows asking a
        // hundred questions are one build.
    ...
    it('keeps its build across components coming and going', () => {
        // Nobody retains it, so nothing releases it: the build lasts as long as the source is the
        // same object, whatever is mounted.
        const { index, getEntities } = createIndex();
        const store = createTestStore([a, b]);
