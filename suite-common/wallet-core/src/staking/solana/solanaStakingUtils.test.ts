...
describe('getSolanaUnstakeAmountBounds', () => {
    it('returns null for non-Solana accounts', () => {
        expect(
            getSolanaUnstakeAmountBounds(networkConfigDeps, buildNonSolanaAccount(), '1'),
        ).toBeNull();
    });
    ...
    it('returns null for zero, negative or invalid amounts', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '0')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '-1')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, 'abc')).toBeNull();
    });
    ...
    it('returns null when there are no active staking accounts', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '0.5')).toBeNull();
    });
    ...
    it('returns null when the requested amount covers the whole active stake', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '1.42')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '2')).toBeNull();
    });
    ...
    it('returns null when both split legs stay above the minimum delegation', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '1')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '1.5')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '2.12')).toBeNull();
    });

    it('suggests only the full amount when the account is too small to split', () => {
        const account = buildSolanaAccount([buildStakingAccount({ stake: `${1.42 * SOL}` })]);

        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '1')).toEqual({
            closestHigher: '1.42',
        });
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '0.5')).toEqual({
            closestHigher: '1.42',
        });
    });
    ...
    it('suggests bounds around the invalid gap of a splittable account', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '2.5')).toEqual({
            closestLower: '2.12',
            closestHigher: '3.12',
        });
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '0.42')).toEqual({
            closestHigher: '1',
        });
    });
    ...
    it('consumes smaller accounts whole before splitting the next one', () => {
        ...
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '1.42')).toBeNull();
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '2')).toEqual({
            closestLower: '1.42',
            closestHigher: '2.42',
        });
        expect(getSolanaUnstakeAmountBounds(networkConfigDeps, account, '3')).toEqual({
            closestLower: '2.42',
            closestHigher: '3.42',
        });
    });
