...
const holdingsOfAsset = (state: AssetHoldingsRootState, assetKey: AssetKey) =>
    assetHoldingsIndex.getBySecondaryKey(state, 'byAsset', assetKey);

const shownHoldings = (state: AssetHoldingsRootState) =>
    assetHoldingsIndex.getAllExcept(state, selectHiddenAssetHoldingKeys(state));
...
describe('the groups a holding is in', () => {
    ...
    it('puts an account’s coin and tokens under that account', () => {
        ...
        expect(
            assetHoldingsIndex.getBySecondaryKey(state, 'byAccountKey', account.key),
        ).toHaveLength(2);
    });
    ...
    it("keeps one wallet's assets apart from another's", () => {
        ...
        const assetKeys = [
            ...assetHoldingsIndex.read(state).getSecondaryIndex('byAsset').keys(),
        ].filter(assetKey => assetKey.startsWith(ALICE));
