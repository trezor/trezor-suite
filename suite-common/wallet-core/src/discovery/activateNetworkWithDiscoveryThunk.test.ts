...
const createState = ({
    ...
    networks: mockNetworksState([networkSymbol]),
});
...
describe(activateNetworkWithDiscoveryThunk.name, () => {
    ...
    it('cancels discovery and restores network visibility when aborted', async () => {
        ...
        const rollbackComplete = createDeferred<void>();
        ...
        await rollbackComplete.promise;
    ...
    it('waits for network activation to settle before rolling it back when aborted', async () => {
        const state = createState();
        const activationComplete = createDeferred<void>();
        const rollbackComplete = createDeferred<void>();

        mockChangeCoinVisibility.mockImplementation(({ shouldBeVisible }) => {
            if (shouldBeVisible) {
                return activationComplete.promise;
            }
