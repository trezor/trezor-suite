...
type WalletSettingsReducerDeps = ExtraDependenciesForReducer & { services: NetworkConfigDeps };

export const prepareWalletSettingsReducer = createReducerWithExtraDeps<
    WalletSettingsState,
    WalletSettingsReducerDeps
>(initialState, (builder, extra) => {
    ...
    builder.addCase(
        ...
        (state, action: ReturnType<typeof walletSettingsActions.changeNetworks>) => {
            const networkSymbols = extra.services.networkModuleRepository.getSupportedNetworks();
            state.enabledNetworks = [...action.payload].sort(
                (a, b) => networkSymbols.indexOf(a) - networkSymbols.indexOf(b),
            );
        },
    );
    builder.addCase(
    ...
    builder.addCase(
        ...
        (
            ...
            state.hideSuspiciousTransactions[symbol] = !state.hideSuspiciousTransactions[symbol];
        },
    );
    builder.addCase(
