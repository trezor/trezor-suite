...
const handleFwHashError = createThunk(
    ...
    ({ device, errorMessage }: { device: Device; errorMessage: string }, { dispatch }) => {
        ...
        if (device.id) dispatch(firmwareActions.setHashInvalid(device.id));
        dispatch(
...
const handleFwHashMismatch = createThunk(
    ...
    (device: Device, { dispatch }) => {
        if (device.id) dispatch(firmwareActions.setHashInvalid(device.id)); // see `handleFwHashError`
...
const handleFwHashValid = createThunk(
    ...
    (device: Device, { dispatch }) => {
        if (device.id) dispatch(firmwareActions.clearInvalidHash(device.id)); // see `handleFwHashError`
