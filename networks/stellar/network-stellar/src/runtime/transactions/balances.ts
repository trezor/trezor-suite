 ...
 *
 * `holder` is not the `account` Horizon stamps on every effect: a `contract_credited` names the
 * classic account that sourced the operation, while the balance belongs to the contract in its own
 * `contract` field. Read as the account's own, the six `contract_*` legs of a swap net it to
 * exactly nothing — and the real holder is also what identifies the counterparty.
 */
...
// Liquidity-pool shares have no asset code and no representation in Suite, so `undefined` means
// "not a balance to report", as against a native `{ asset: undefined }`.
const readAsset = (
    assetType: string,
    assetCode?: string,
    assetIssuer?: string,
...
/** The movements Horizon states outright: a credit or a debit is the ledger's own account. */
...
/**
 * A crossing offer moves value without Horizon crediting or debiting anything, so the trade effect
 * is the only record of it; its `sold`/`bought` are written from the effect account's side.
 * `liquidity_pool_trade` is deliberately not read: its sides are the pool's, the inverse of the
 * account's, and inverting them here would invent a direction.
 */
...
/**
 * Nets the effects of one transaction into a single movement per holder and asset. Trades are
 * weighed per operation: Horizon reports a path payment's hops as trades *on top of* the credit and
 * debit the payment produced, so counting both would double the swap, while a crossing offer
 * produces nothing but trades. A trade therefore only speaks for an operation that moved nothing.
 */
