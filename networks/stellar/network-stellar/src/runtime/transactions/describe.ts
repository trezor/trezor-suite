...
import type { StellarOperationType } from '../../types/operations';
...
// Horizon marshals a call that moved nothing as `null`, though the SDK types an array.
...
// Operations that never move a balance, so reporting no movement is the whole truth.
...
/** Describes a transaction by its netted effects; with none it stays `unknown` (fails closed). */
...
/** Maps one transaction's operations and effects onto what `transformTransaction` consumes. */
export const describeTransaction = (
    ...
    if (!operation || operations.length !== 1) {
        // Several operations net into one movement; naming one of them would be arbitrary.
    ...
    switch (operation.type) {
        ...
        case Horizon.HorizonApi.OperationResponseType.payment: {
            ...
            // A liquidity-pool share has no representation here; the effects may still describe it.
        ...
        case Horizon.HorizonApi.OperationResponseType.changeTrust: {
            // A liquidity-pool share carries no asset code, so only the operation can be named.
        ...
        case Horizon.HorizonApi.OperationResponseType.invokeHostFunction: {
            ...
            // Balance changes cover classic assets only, so the effects come along.
        ...
        // Nothing has moved yet: the value arrives only if a claimant claims it (the spam shape too).
