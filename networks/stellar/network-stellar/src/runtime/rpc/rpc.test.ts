...
describe('rpc/ledgerKeys', () => {
    ...
    it('builds a trustline key matching the recorded entry', () => {
        ...
        const data = toEntryData(fixtures.TRUSTLINE_ALPHANUM4);

        if (data.type !== 'trustline' || key.type !== 'trustline') {
            throw new Error('expected a trustline entry and key');
        }
...
describe('rpc/submit', () => {
    ...
    it('reports a transaction that already applied as sent, not rejected', async () => {
        const server = asServer({
            sendTransaction: () => Promise.resolve(sent('ERROR', { errorResult: failedResult })),
    ...
    it('still reports a rejection the ledger has no transaction for', async () => {
        const server = asServer({
            sendTransaction: () => Promise.resolve(sent('ERROR', { errorResult: failedResult })),
    ...
    it('stops waiting out congestion once the transaction has applied', async () => {
        const sendTransaction = jest.fn().mockResolvedValue(sent('TRY_AGAIN_LATER'));
        const server = asServer({
            sendTransaction,
            getTransaction: () => Promise.resolve(got('SUCCESS')),
        });

        // No timers to advance: the retry budget is never spent, so the send resolves outright.
        await expect(submitTransaction({ server, transaction })).resolves.toBe(HASH);
    ...
    it('retries a congested node before giving up on the send', async () => {
        ...
        // Not applied while the node is congested, so the retry budget is what settles this one.
        const getTransaction = jest
            .fn()
            .mockResolvedValueOnce(got('NOT_FOUND'))
            .mockResolvedValue(got('SUCCESS'));
        const server = asServer({ sendTransaction, getTransaction });
    ...
    it('reports a send a congested node never queued, rather than the hash', async () => {
        ...
        // The ledger is asked whether the send landed anyway — what must not happen is polling it
        // out to the budget and reporting the hash of a send the node never took.
