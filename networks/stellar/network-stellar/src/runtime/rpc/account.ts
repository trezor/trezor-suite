...
/**
 * Reads an account's native balance, sequence, reserve inputs and trustline balances in one batched
 * `getLedgerEntries`, in stroops. An entry that does not exist is omitted rather than reported
 * empty, which is exactly the "does this trustline exist" signal — and, for the account key
 * itself, "not funded yet".
 */
