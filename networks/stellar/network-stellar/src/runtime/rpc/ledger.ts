...
// The base reserve has changed once in the network's history, so the protocol constant is a safe
// answer when a node omits the header; losing the ledger head over a cosmetic field would not be.
...
/** One RPC call replaces Horizon's separate ledger and base-reserve reads. */
