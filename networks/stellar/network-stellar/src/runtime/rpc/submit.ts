...
// Only `txFailed`/`txSuccess` carry per-operation results, so the read is guarded rather than
// branched on each of the two dozen codes.
const readOperationResultCode = (result: xdr.TransactionResult) => {
    try {
        const inner = result.result;

        // Only these two arms carry per-operation results.
