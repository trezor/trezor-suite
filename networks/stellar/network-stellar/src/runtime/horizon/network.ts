...
/**
 * Reads a version string from Horizon, for when Stellar RPC is unreachable. The RPC path reports
 * the RPC node's own version, so the two are not interchangeable — the Core version is the
 * closest equivalent, and it identifies the network the data came from.
 */
...
/**
 * Reads which network Horizon serves, for when Stellar RPC is unreachable. Both protocols report
 * the same passphrase for the same network, so this one is interchangeable with the RPC read —
