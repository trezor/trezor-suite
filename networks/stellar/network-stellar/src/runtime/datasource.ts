...
/**
 * Reads from the preferred source, standing in with the other when the first fails to answer at
 * all. Only a thrown error triggers the stand-in: a read that answers "this account has no ledger
 * entry" is authoritative, so a funded-account check is never second-guessed against Horizon, which
 * knows only what it has indexed. When the stand-in fails too the first error is rethrown.
 */
...
/**
 * The one seam the blockchain-link worker talks to. Stellar RPC answers everything about account
 * state; Horizon is left with transaction history and, for now, trustline discovery.
 */
export const createStellarDataSource = (
    ...
    // source will do — see `STELLAR_LEDGER_HEAD_SOURCE` for why Horizon is asked first.
    readLatestLedger: () =>
        headSource === 'horizon'
            ? readWithFallback(
    ...
    // Sending is RPC-only, so there is nothing to degrade to here.
    readInclusionFee: () => readInclusionFee(api.rpc),
    readAccountState: async ({ descriptor, knownAssets }) => {
        // A Horizon 404 only says Horizon has not seen the account; the missing ledger entry is
        // what decides, so the RPC read runs either way. The record is kept for the fallback.
        const record =
            trustlineDiscovery === 'rpc'
                ? undefined
                : await fetchAccountRecord(api.horizon, descriptor);
        // Over RPC the caller's list is all there is, but Horizon enumerating nothing is an answer.
        ...
        return readWithFallback(
            ...
            async () => {
                ...
                // Horizon not having the account is not proof it is unfunded, so the outage is
