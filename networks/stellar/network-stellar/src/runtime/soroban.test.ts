...
describe('readSep41Tokens', () => {
    // Each test uses its own contracts, `contractList` below included: metadata is cached for the
    // lifetime of the module, so a shared contract would undercount another test's simulations.
...
describe('readSep41Tokens fallback pool', () => {
    // The budget test drives the clock; a read that loses its race must not outlive it.
    ...
    const countingServer = (onCall?: (calls: number) => void) => {
        ...
        return {
            // The ledger read answers nothing, so every contract falls through to the pool.
            server: {
                simulateTransaction,
    ...
    it('stops reading when the budget for the whole list runs out', async () => {
        ...
        // Time passes mid-flight, so the pool finds the budget spent when it comes back for more.
