...
describe(getContractTokenMetadata.name, () => {
...
describe(readSep41Tokens.name, () => {
    // Contract ids are unique per test: metadata is cached for the lifetime of the module.
    ...
    afterEach(() => {
        // In teardown, so a failing assertion cannot leave fake timers installed.
        jest.useRealTimers();
    });
...
describe(prepareContractTransaction.name, () => {
...
describe(serializeSignedTransaction.name, () => {
    ...
    it('serializes the envelope the way the SDK signs it', () => {
        const signature = transfer().getKeypairSignature(signer);
        const reference = transfer();
        reference.addSignature(signer.publicKey(), signature);
        ...
        ).toBe(reference.toEnvelope().toXdr('hex'));
    });
});
...
describe('readSep41Tokens fallback pool', () => {
    ...
    it('drops a cached read that ran out of time instead of waiting on it again', async () => {
        const [contract = ''] = contractList(1, 96);
        // Pinned, so every read gets its full budget however far the timers are advanced.
        jest.spyOn(Date, 'now').mockImplementation(() => 0);

        const simulateTransaction = jest.fn(() => new Promise<never>(() => {}));
        const server = {
            simulateTransaction,
            getLedgerEntries: () => Promise.resolve({ entries: [], latestLedger: 1 }),
        } as unknown as StellarRpcServer;
        ...
        // The metadata read is still in flight and still cached; a later account read must neither
        // wait on it unbounded nor keep it.
