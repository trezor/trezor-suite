...
describe(getDecompiledMessage.name, () => {
    ...
    // Skipped on this branch: the addressTableLookups early-return in getDecompiledMessage comes
    // from 467c4002c3 (fix(connect): skip solana precompose for transactions with address lookup
    // tables), which has not been cherry-picked to release/26.9.
    it.skip('returns undefined for a v0 transaction with address lookup tables', () => {
