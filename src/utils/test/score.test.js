...
describe('Score Utils', () => {
    it('should not be in probability in beta', () => {
        const result = isInProbability(true, 0.21, 0.30);
        expect(result).toBe(true);
    });

    it('should not be in probability with higher score', () => {
        const result = isInProbability(false, 0.21, 0.30);
        expect(result).toBe(false);
    });

    it('should be in probability with lower score', () => {
        const result = isInProbability(false, 0.21, 0.10);
        expect(result).toBe(true);
    });

    it('should be in probability if score is equal', () => {
        const result = isInProbability(false, 0.51, 0.51);
        expect(result).toBe(true);
    });
