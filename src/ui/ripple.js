...
const discovery = async () => {
    const account = await window.TrezorConnect.rippleGetAccountInfo({
        bundle: [
            { path: "m/44'/144'/0'/0/0", showOnTrezor: false },
            { path: "m/44'/144'/1'/0/0", showOnTrezor: false },
            { path: "m/44'/144'/2'/0/0", showOnTrezor: false },
    ...
    const ERROR: 'blockchain-error' = 'blockchain-error';
    const CONNECT: 'blockchain-connect' = 'blockchain-connect';
    const BLOCK: 'blockchain-block' = 'blockchain-block';
    const NOTIFICATION: 'blockchain-notification' = 'blockchain-notification';

    window.TrezorConnect.on(BLOCK, (a) => {
        console.warn("BLOCK", a)
    })

    window.TrezorConnect.on(NOTIFICATION, (a) => {
        console.warn("NOTIFICATION", a)
    })

    const subs = await window.TrezorConnect.blockchainSubscribe({
        coin: 'xrp'
    });
