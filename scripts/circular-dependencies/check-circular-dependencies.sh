...
if ! npx madge --circular --extensions ts,tsx,js,jsx --exclude "node_modules|lib|libDev" packages suite suite-native suite-common | tail -n +2 | sed 's/^[0-9][0-9]*) *//' | sort > "$TMP_FILE"; then
...
if ! git diff --no-index --color=always "$SNAPSHOT_FILE" "$TMP_FILE"; then
  ...
  echo "  npx madge --circular --extensions ts,tsx,js,jsx --exclude \"node_modules|lib|libDev\" packages suite suite-native suite-common | tail -n +2 | sed 's/^[0-9][0-9]*) *//' | LC_ALL=C sort > $SNAPSHOT_FILE"
  rm -f "$TMP_FILE"
  exit 1
fi
