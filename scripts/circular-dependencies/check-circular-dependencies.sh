...
export GIT_PAGER=cat  # prevent diffs in vim, ...

SNAPSHOT_FILE="scripts/circular-dependencies/madge-snapshot.txt"
TMP_FILE="$(mktemp)"

# Run madge, allow it to fail
if ! npx madge --circular --extensions ts,tsx,js,jsx --exclude "node_modules|lib|libDev" packages suite suite-native suite-common | tail -n +2 | sed 's/^[0-9]\+)//' | sort > "$TMP_FILE"; then
