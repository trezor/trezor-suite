...
- **Web and desktop are compiled tree by tree. Compiled today: `suite-common/`** (wave 1a). Every
  other tree in the bundle — `packages/suite`, `suite/*`, `packages/components`,
  `packages/product-components`, `packages/react-utils`, `packages/icons`, `packages/suite-web` and
  `packages/suite-desktop-ui` — still ships uncompiled, so manual memoization stays its only runtime
  mechanism there. The compiler's lint rules below apply everywhere regardless.
